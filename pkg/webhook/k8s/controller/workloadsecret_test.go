...
import (
	...
	mockca "istio.io/istio/pkg/webhook/ca/mock"
	"istio.io/istio/pkg/webhook/ca/util"
	mockutil "istio.io/istio/pkg/webhook/ca/util/mock"
	"istio.io/istio/pkg/webhook/k8s/secret"
)
...
var (
	...
	istioTestSecret = secret.BuildSecret("test", "istio.test", "test-ns", certChain, caKey, rootCert, nil, nil, IstioSecretType)
)
	...
	testCases := map[string]struct {
		...
		"updating service accounts deletes old secret and creates a new one": {
			...
			expectedActions: []ktesting.Action{
				...
				ktesting.NewCreateAction(gvr, "new-ns", secret.BuildSecret("new-name", "istio.new-name", "new-ns", certChain, caKey, rootCert, nil, nil, IstioSecretType)),
			},
			gracePeriodRatio: defaultGracePeriodRatio,
			shouldFail:       false,
		},
		...
		"adding webhook service account": {
			...
			expectedActions: []ktesting.Action{
				ktesting.NewCreateAction(gvr, "test-ns",
					secret.BuildSecret("test", sidecarInjectorSvcAccount, "test-ns", certChain, caKey, rootCert, nil, nil, IstioSecretType)),
			},
			gracePeriodRatio: defaultGracePeriodRatio,
			shouldFail:       false,
		},
...
func TestSecretContent(t *testing.T) {
	...
	secret := secret.BuildSecret(saName, GetSecretName(saName), saNamespace, nil, nil, nil, nil, nil, IstioSecretType)
...
func TestDeletedIstioSecret(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"Recover secret for existing service account": {
			secret: secret.BuildSecret("test-sa", "istio.test-sa", "test-ns", nil, nil, nil, nil, nil, IstioSecretType),
			expectedActions: []ktesting.Action{
				ktesting.NewGetAction(saGvr, "test-ns", "test-sa"),
				ktesting.NewCreateAction(scrtGvr, "test-ns", secret.BuildSecret("test-sa", "istio.test-sa", "test-ns", nil, nil, nil, nil, nil, IstioSecretType)),
			},
		},
		"Do not recover secret for non-existing service account in the same namespace": {
			secret: secret.BuildSecret("test-sa2", "istio.test-sa2", "test-ns", nil, nil, nil, nil, nil, IstioSecretType),
			expectedActions: []ktesting.Action{
				ktesting.NewGetAction(saGvr, "test-ns", "test-sa2"),
			},
		},
		"Do not recover secret for service account in different namespace": {
			secret: secret.BuildSecret("test-sa", "istio.test-sa", "test-ns2", nil, nil, nil, nil, nil, IstioSecretType),
			expectedActions: []ktesting.Action{
