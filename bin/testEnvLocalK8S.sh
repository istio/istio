...
export LOCAL_GOOS=${LOCAL_GOOS:-linux}

export GOPATH=${TOP}
export PATH=${GOPATH}/bin:${PATH}
export OUT=${TOP}/out
export ISTIO_OUT=${ISTIO_OUT:-${TOP}/out/${LOCAL_GOOS}_amd64/release}
...
function getDeps() {
   ...
   if [ ! -f $TOP/bin/kubectl ] ; then
     if [ -f /usr/local/bin/kubectl ] ; then
       ...
       curl -Lo $TOP/bin/kubectl https://storage.googleapis.com/kubernetes-release/release/${K8S_VER}/bin/${LOCAL_GOOS}/amd64/kubectl && chmod +x $TOP/bin/kubectl
     fi
   fi
   if [ ! -f $TOP/bin/kube-apiserver ] ; then
     if [ -f /usr/local/bin/kube-apiserver ] ; then
       ...
       curl -Lo ${TOP}/bin/kube-apiserver https://storage.googleapis.com/kubernetes-release/release/${K8S_VER}/bin/${LOCAL_GOOS}/amd64/kube-apiserver && chmod +x ${TOP}/bin/kube-apiserver
     fi
   fi
