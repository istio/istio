...
func (s *source) handleEvent(c resource.EventKind, obj interface{}) {
	...
	if uns, ok := obj.(*unstructured.Unstructured); ok {
		...
		u.SetGroupVersionKind(schema.GroupVersionKind{
			Group:   s.spec.Group,
			Version: sourceSchema.GetAPIVersion(&s.spec),
			Kind:    s.spec.Kind,
		})
	}
...
func ConvertAndLog(cfg *converter.Config, spec sourceSchema.ResourceSpec, key resource.FullName,
	...
	if err != nil {
		...
		stats.RecordConverterResult(false, sourceSchema.GetAPIVersion(&spec), spec.Group, spec.Kind)
		return nil, err
	}
	stats.RecordConverterResult(true, sourceSchema.GetAPIVersion(&spec), spec.Group, spec.Kind)
	return entries, nil
}
