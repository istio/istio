...
type IstioConfigStore interface {
	...
	EgressRules() map[string]*routerule.EgressRule
...
func ResolveHostname(meta ConfigMeta, svc *routerule.IstioService) string {
	out := svc.Name
...
func MatchSource(meta ConfigMeta, source *routerule.IstioService, instances []*ServiceInstance) bool {
	if source == nil {
		return true
	}
...
func SortRouteRules(rules []Config) {
	...
	sort.Slice(rules, func(i, j int) bool {
		...
		irule, _ := rules[i].Spec.(*routerule.RouteRule)
		jrule, _ := rules[j].Spec.(*routerule.RouteRule)
...
func (store *istioConfigStore) EgressRules() map[string]*routerule.EgressRule {
	out := make(map[string]*routerule.EgressRule)
	rs, err := store.List(EgressRule.Type, "")
	if err != nil {
		return nil
	}
	for _, r := range rs {
		if rule, ok := r.Spec.(*routerule.EgressRule); ok {
			out[r.Key()] = rule
		}
	}
	return out
...
func (store *istioConfigStore) Policy(instances []*ServiceInstance, destination string, labels Labels) *Config {
	...
	var found bool
	for _, config := range configs {
		policy := config.Spec.(*routerule.DestinationPolicy)
...
func mixerToProxyIstioService(in *mccpb.IstioService) *routerule.IstioService {
	return &routerule.IstioService{
		Name:      in.Name,
