...
import (
	...
	config "istio.io/istio/mixer/adapter/circonus/config"
...
// NewCirconusNoSessionServer creates a new no session server based on given args.
func NewCirconusNoSessionServer(addr uint16, poolSize int, c *Cert) (*NoSession, error) {
	saddr := fmt.Sprintf(":%d", addr)

	gp := pool.NewGoroutinePool(poolSize, false)
	inf := circonus.GetInfo()
	s := &NoSession{
		builder:    inf.NewBuilder(),
		env:        handler.NewEnv(0, "circonus-nosession", gp),
