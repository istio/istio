...
func (c *kubeComponent) getAddressInner(env *kube.Environment, ns string, port int) (interface{}, bool, error) {
...
func getHTTPSAddressInner(env *kube.Environment, ns string) (interface{}, bool, error) {
	if env.Settings().Minikube {
		pods, err := env.Accessors[0].GetPods(ns, fmt.Sprintf("istio=%s", istioLabel))
		if err != nil {
			return nil, false, err
		}
		...
		svc, err := env.Accessors[0].GetService(ns, serviceName)
		if err != nil {
			return nil, false, err
		}
...
func (c *kubeComponent) HTTPAddress() net.TCPAddr {
	address, err := retry.Do(func() (interface{}, bool, error) {
		return c.getAddressInner(c.env, c.namespace, 80)
...
func (c *kubeComponent) TCPAddress() net.TCPAddr {
	address, err := retry.Do(func() (interface{}, bool, error) {
		return c.getAddressInner(c.env, c.namespace, 15029)
	}, retryTimeout, retryDelay)
	if err != nil {
		return net.TCPAddr{}
	}
...
func (c *kubeComponent) adminRequest(path string) (string, error) {
	pods, err := c.env.Accessors[0].GetPods(c.namespace, "istio=ingressgateway")
	if err != nil {
	...
	return c.env.Accessors[0].Exec(podNs, podName, proxyContainerName, command)
}
