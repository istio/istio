...
func (j *mergejoin2[T]) Metadata() Metadata {
	return j.metadata
}

// nolint: unused // (not true, its to implement an interface)
func (j *mergejoin2[T]) augment(a any) any {
	// not supported in this collection type
	return a
}

// nolint: unused // (not true, its to implement an interface)
func (j *mergejoin2[T]) name() string { return j.collectionName }

// nolint: unused // (not true, its to implement an interface)
func (j *mergejoin2[T]) uid() collectionUID { return j.id }

func (j *mergejoin2[T]) Synced() Syncer {
...
func (j *nestedjoin2[T]) HasSynced() bool {
	return j.syncer.HasSynced()
}

func (j *nestedjoin2[T]) WaitUntilSynced(stop <-chan struct{}) bool {
	return j.syncer.WaitUntilSynced(stop)
}

// nolint: unused // (not true)
func (j *mergejoin2[T]) index(name string, extract func(o T) []string) indexer[T] {
	j.mu.Lock()
	defer j.mu.Unlock()
...
func (j *mergejoin2[T]) HasSynced() bool {
	return j.syncer.HasSynced()
}

func (j *mergejoin2[T]) WaitUntilSynced(s <-chan struct{}) bool {
	return j.syncer.WaitUntilSynced(s)
}

func (j *mergejoin2[T]) List() []T {
	j.mu.RLock()
	defer j.mu.RUnlock()
...
func (j *mergejoin2[T]) GetKey(k string) *T {
	j.mu.RLock()
	defer j.mu.RUnlock()
...
func (j *mergejoin2[T]) refreshEventsLocked(items []Event[T]) []Event[T] {
...
func (j *mergejoin2[T]) calculateMerged(k string) *T {
	var found []T
