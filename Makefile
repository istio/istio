...
${ISTIO_OUT}/archive: istioctl-all LICENSE README.md install/updateVersion.sh release/create_release_archives.sh samples/bookinfo/scripts/generate_platform_specific_routing.sh
	...
	install/updateVersion.sh -a "$(ISTIO_DOCKER_HUB),$(VERSION)" \
                                 -P "$(ISTIO_URL)/deb" \
                                 -d "${ISTIO_OUT}/archive"
# remove the following line after v1alpha2 are removed
	pushd samples/bookinfo; scripts/generate_platform_specific_routing.sh; popd
