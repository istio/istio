...
// Reconcile implements ControllerViewHandler interface
...
func (v *MeshResourceView) Run(stop <-chan struct{}) {
	log.Info("Aggregated Controller terminated")
...
// SetCacheEvictionHandler implements model.UpdateNotifier
...
func (v *MeshResourceView) reconcileServices(r *Registry, cvp string, rl resourceLabels, expectedServices []*model.Service) {
	...
	if log.DebugEnabled() {
		log.Debugf("Expected: %s Actual %s",
...
func (v *MeshResourceView) reconcileService(cp string, k resourceKey, s *model.Service, e model.Event) {
	if log.DebugEnabled() {
		log.Debugf("Svc event '%s' for service '%s'", e.String(), s.Hostname)
	}
