...
func ValidateCSR(csrPEM []byte, subjectIDs []string) bool {
	...
	// Validate the CSR is generated by Istio agent and doesn't have arbitrary fields.
	// See generate_csr.go GenCSRTemplate and secretcache.go for the counterpart and options
	goodcsr := &x509.CertificateRequest{}
	if len(csr.Subject.Organization) > 1 {
		return false
	}
	if len(csr.Subject.Organization) == 1 {
		goodcsr.Subject.Organization = []string{csr.Subject.Organization[0]}
	}
	goodsubj, err := asn1.Marshal(goodcsr.Subject.ToRDNSequence())
	csrsubj, err := asn1.Marshal(csr.Subject.ToRDNSequence())
	if !bytes.Equal(goodsubj, csrsubj) {
		return false
	}
