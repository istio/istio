...
func setupFilterChains(authnPolicy *authn.Policy, sdsUdsPath string, sdsUseTrustworthyJwt, sdsUseNormalJwt bool,
	protocol model.Protocol, meta map[string]string) []plugin.FilterChain {
	...
	if mtls.GetMode() == authn.MutualTls_PERMISSIVE {
		// skip setting up permissive TLS for MySQL. Sniffing wont work because in mysql protocol, the server is expected
		// to respond first
		if protocol == model.ProtocolMySQL {
			log.Debug("Skipping setup of tls sniffing filter for inbound mysql listener")
			return nil
		}
...
func (Plugin) OnInboundFilterChains(in *plugin.InputParams) []plugin.FilterChain {
	...
	return setupFilterChains(authnPolicy, in.Env.Mesh.SdsUdsPath, in.Env.Mesh.EnableSdsTokenMount, in.Env.Mesh.SdsUseK8SSaJwt,
		in.Port.Protocol, in.Node.Metadata)
}
