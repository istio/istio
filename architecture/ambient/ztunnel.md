...
A secondary goal was to enable was to enable a smoother on-ramp from "Zero" to "Getting some value".
...
* Be lightweight enough to not limit adoption.
  * This puts a much tighter budget on CPU, memory, latency, and throughput requirements than traditional Istio.
...
This works out to be more efficient in common cases where policies change must less often than workloads.
This only applies for selector-based policies; namespaced and global policies can be handled without needing to list them out.
...
* All traffic *ingressing* a pod on port 15008 in the mesh should be redirected to the node-local ztunnel on port 15008 (more on this later).
* All other traffic *ingressing* a pod in the mesh should be redirected to the node-local ztunnel on port 15006.

TODO: fill in implementation details of how redirection is actually implemented.

## HBONE

Along with pass-through traffic, Ztunnel supports the "HBONE" protocol.
...
SNI is not used because it is illegal to use IP's in SNI, and there is no other existing standard format to represent what we need to.
...
If the target destination has a waypoint, we enforce the request is coming from that waypoint. Otherwise, the request is rejected.
...
However, the identies of the certificates will be of the actual user workloads, not Ztunnel's own identity.
