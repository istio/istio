...
func (i *operatorComponent) Close() (err error) {
	...
	if i.settings.DeployIstio {
		err = i.environment.Accessors[0].DeleteNamespace(i.settings.SystemNamespace)
		if err == nil {
			err = i.environment.Accessors[0].WaitForNamespaceDeletion(i.settings.SystemNamespace)
		}
		...
		_ = i.environment.Accessors[0].DeleteValidatingWebhook(DefaultValidatingWebhookConfigurationName(i.settings))
		_ = i.environment.Accessors[0].DeleteMutatingWebhook(DefaultMutatingWebhookConfigurationName)
	}
	return
}
...
func (i *operatorComponent) Dump() {
	...
	deployment.DumpPodState(d, i.settings.SystemNamespace, i.environment.Accessors[0])
	deployment.DumpPodEvents(d, i.settings.SystemNamespace, i.environment.Accessors[0])

	pods, err := i.environment.Accessors[0].GetPods(i.settings.SystemNamespace)
	if err != nil {
	...
	for _, pod := range pods {
		for _, container := range pod.Spec.Containers {
			l, err := i.environment.Accessors[0].Logs(pod.Namespace, pod.Name, container.Name, false /* previousLog */)
			if err != nil {
...
func deploy(ctx resource.Context, env *kube.Environment, cfg Config) (Instance, error) {
	...
	if !cfg.SkipWaitForValidationWebhook {
		...
		if _, _, err = env.Accessors[0].WaitUntilServiceEndpointsAreReady(cfg.SystemNamespace, "istio-pilot"); err != nil {
		...
		err = waitForValidationWebhook(env.Accessors[0], cfg)
		if err != nil {
			i.Dump()
			return nil, err
		}
