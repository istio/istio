...
test.integration.%.kube: | $(JUNIT_REPORT) check-go-tag
	PATH=${PATH}:${ISTIO_OUT} $(GO) test -p 1 ${T} -tags=integ $(shell go list -tags=integ ./tests/integration/$(subst .,/,$*)/... | grep -v /multicluster/centralistio | grep -v multicluster/centralremotekubeconfig) -timeout 30m \
