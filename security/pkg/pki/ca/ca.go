...
func NewSelfSignedIstioCAOptions(ctx context.Context,
	...
	err = b.RetryWithContext(ctx, func() error {
		...
		if err == nil {
			// If the secret is not generated by istio and named "cacerts", ignore it.
			if _, ok := caSecret.Data[IstioGenerated]; !ok && caSecret.Name == ExternalCASecret {
				return fmt.Errorf("secret %s/%s is not generated by istio. Ignoring", namespace, caCertName)
			}
		...
		if apierror.IsNotFound(err) {
			pkiCaLog.Infof("CASecret %s not found, will create one called %s", caCertName, ExternalCASecret)
