...
// TestCollectionNestedSubscribe tests that a collection can subscribe to another collection
// without having to fetch/list the other collection's elements
func TestCollectionSubscribe(t *testing.T) {
	stop := test.NewStop(t)
	opts := testOptions(t)
	c := kube.NewFakeClient()
	kpc := kclient.New[*corev1.Pod](c)
	...
	PredefinedSimplePods := krt.NewCollection(pods, func(ctx krt.HandlerContext, i *corev1.Pod) *SimplePod {
		krt.Subscribe(ctx, SimplePods)
		counter.Inc()
		return &SimplePod{
			Named: Named{
				Namespace: "namespace",
				Name:      "name" + strconv.Itoa(int(counter.Load())),
			},
			IP: "1.2.3.4",
		}
	}, opts.WithName("NestedSimplePod")...)
	tt := assert.NewTracker[string](t)
	PredefinedSimplePods.Register(TrackerHandler[SimplePod](tt))
	...
	pod.Status = corev1.PodStatus{PodIP: "1.2.3.4"}
	pc.UpdateStatus(pod)
	tt.WaitUnordered("add/namespace/name2", "delete/namespace/name1", "add/namespace/name3", "delete/namespace/name2")

	pod.Status.PodIP = "1.2.3.5"
	pc.UpdateStatus(pod)
	tt.WaitUnordered("add/namespace/name4", "delete/namespace/name3", "add/namespace/name5", "delete/namespace/name4")

	pc.Delete(pod.Name, pod.Namespace)
	tt.WaitUnordered("delete/namespace/name5")
}
