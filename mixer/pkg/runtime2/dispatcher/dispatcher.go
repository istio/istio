...
// RoutingContext is the current dispatching context, based on a config snapshot. As config changes, the current/live
// RoutingContext also changes.
...
func (d *Dispatcher) Check(ctx context.Context, bag attribute.Bag) (*adapter.CheckResult, error) {
	s := d.beginSession(istio_mixer_v1_template.TEMPLATE_VARIETY_CHECK, ctx, bag)
...
func (d *Dispatcher) Report(ctx context.Context, bag attribute.Bag) error {
	s := d.beginSession(istio_mixer_v1_template.TEMPLATE_VARIETY_REPORT, ctx, bag)
...
func (d *Dispatcher) Quota(ctx context.Context, bag attribute.Bag, qma *runtime.QuotaMethodArgs) (*adapter.QuotaResult, error) {
	s := d.beginSession(istio_mixer_v1_template.TEMPLATE_VARIETY_QUOTA, ctx, bag)
...
func (d *Dispatcher) Preprocess(ctx context.Context, bag attribute.Bag, responseBag *attribute.MutableBag) error {
	s := d.beginSession(istio_mixer_v1_template.TEMPLATE_VARIETY_ATTRIBUTE_GENERATOR, ctx, bag)
	s.responseBag = responseBag

	err := d.dispatch(s)
	if err == nil {
		responseBag = s.responseBag
		err = s.err
	}
...
func (d *Dispatcher) beginSession(variety istio_mixer_v1_template.TemplateVariety, ctx context.Context, bag attribute.Bag) *session {
	s := d.sessionPool.get()
	s.start = time.Now()
	s.variety = variety
	s.ctx = ctx
...
func (d *Dispatcher) dispatch(session *session) error {
	// Capture the routing locally. It can change underneath us. We also need to decRef before
	...
	// lookup.
	// Allocates 16 bytes due to bag lookup.
	...
	if err != nil {
		...
		log.Warnf("unable to determine identity attr value: '%v', variety='%d'", err, session.variety)
		return err
	}
	...
	// Allocates 80 bytes due to bag lookup and update of the context.
	session.ctx = d.updateContext(session.ctx, session.bag)

	// TODO: This is for old-style metadata based policy decisions. This should be eventually removed.
	// Allocates 64 bytes due to bag lookup.
	...
	for _, destination := range destinations.Entries() {
		for _, inputs := range destination.InputGroups {
			if !inputs.Matches(session.bag) || inputs.ResourceType.IsTCP() != tcp {
				continue
			}
			...
			if session.variety == istio_mixer_v1_template.TEMPLATE_VARIETY_REPORT {
				state = d.statePool.get(session, destination)
			}

			ndestination++
			for j, input := range inputs.Builders {
				var instance interface{}
				// allocates 16 bytes minimum, per instance
				...
				if session.variety == istio_mixer_v1_template.TEMPLATE_VARIETY_ATTRIBUTE_GENERATOR {
					state.mapper = inputs.Mappers[j]
				}
			...
			if session.variety == istio_mixer_v1_template.TEMPLATE_VARIETY_REPORT {
				// Dispatch for multi-instances dispatches (i..e report)
				d.dispatchToHandler(state)
			}
		}
	}
	...
	r.DecRef()
	updateRequestCounters(time.Since(session.start), ndestination, ninputs, err != nil)
