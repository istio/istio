...
func (h *HelmReconciler) Prune(manifests name.ManifestMap, all bool) (*v1alpha1.InstallStatus, error) {
	// prune for the IOP contains revision
	iop := h.iop
	if iop != nil && iop.Spec != nil {
		rev := iop.Spec.Revision
		if rev != "" {
			return h.PruneControlPlaneByRevisionWrapper(iop.Spec.Namespace, rev)
		}
	}
	return nil, h.runForAllTypes(func(labels map[string]string, objects *unstructured.UnstructuredList) error {
...
// PruneControlPlaneByRevisionWrapper is wrapper to remove specific control plane revision.
