...
func TestOrdering(t *testing.T) {
	...
	q := NewFixedDelayQueue(1 * time.Microsecond)
...
func TestRunningLatestTask(t *testing.T) {
	option := BackOffOption{
		InitialInterval: 1 * time.Microsecond,
		MaxInterval:     5 * time.Microsecond,
	}
	...
	wg.Wait()
	if data != 3 {
		t.Errorf("should run the latest task handler")
	}
}
