...
func (s *Server) initConfigController(args *PilotArgs) error {
	...
	if hasKubeRegistry(args.RegistryOptions.Registries) && meshConfig.IngressControllerMode != meshconfig.MeshConfig_OFF {
		...
		s.addTerminatingStartFunc(func(stop <-chan struct{}) error {
			leaderelection.
				NewLeaderElection(args.Namespace, args.PodName, leaderelection.IngressController, args.Revision, s.kubeClient, s.defaultWatcher).
...
func (s *Server) initK8SConfigStore(args *PilotArgs) error {
	...
	if features.EnableGatewayAPI {
		...
		s.addTerminatingStartFunc(func(stop <-chan struct{}) error {
			leaderelection.
				NewLeaderElection(args.Namespace, args.PodName, leaderelection.GatewayStatusController, args.Revision, s.kubeClient, s.defaultWatcher).
				AddRunFunction(func(leaderStop <-chan struct{}) {
		...
		if features.EnableGatewayAPIDeploymentController {
			s.addTerminatingStartFunc(func(stop <-chan struct{}) error {
				leaderelection.
					NewLeaderElection(args.Namespace, args.PodName, leaderelection.GatewayDeploymentController, args.Revision, s.kubeClient, s.defaultWatcher).
					AddRunFunction(func(leaderStop <-chan struct{}) {
...
func (s *Server) initInprocessAnalysisController(args *PilotArgs) error {
	...
	s.addStartFunc(func(stop <-chan struct{}) error {
		go leaderelection.
			NewLeaderElection(args.Namespace, args.PodName, leaderelection.AnalyzeController, args.Revision, s.kubeClient, s.defaultWatcher).
...
func (s *Server) initStatusController(args *PilotArgs, writeStatus bool) {
	...
	if writeStatus {
		s.addTerminatingStartFunc(func(stop <-chan struct{}) error {
			leaderelection.
				NewLeaderElection(args.Namespace, args.PodName, leaderelection.StatusController, args.Revision, s.kubeClient, s.defaultWatcher).
				AddRunFunction(func(stop <-chan struct{}) {
