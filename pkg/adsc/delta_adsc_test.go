...
type testDeltaAdscRunServer struct{}

func (t *testDeltaAdscRunServer) StreamAggregatedResources(server discovery.AggregatedDiscoveryService_StreamAggregatedResourcesServer) error {
	return nil
}

var DeltaStreamHandler func(stream discovery.AggregatedDiscoveryService_DeltaAggregatedResourcesServer) error

func (t *testDeltaAdscRunServer) DeltaAggregatedResources(stream discovery.AggregatedDiscoveryService_DeltaAggregatedResourcesServer) error {
	return DeltaStreamHandler(stream)
}

func TestDeltaADSC_Run(t *testing.T) {
	type testCase struct {
		desc                 string
		inAdsc               *DeltaADSC
		deltaStreamHandler   func(server discovery.AggregatedDiscoveryService_DeltaAggregatedResourcesServer) error
		expectedADSResources *DeltaADSC
	...
	type testDesc struct {
		desc                 string
		deltaInitialRequests []*discovery.DeltaDiscoveryRequest
	...
	descs := []testDesc{
		...
		},
		{
			...
			deltaInitialRequests: ConfigInitialDeltaRequests(),
	...
	for _, item := range descs {
		...
		expected := map[string]*discovery.DeltaDiscoveryResponse{}
		for _, request := range desc.deltaInitialRequests {
			if desc.excludedResource != "" && request.TypeUrl == desc.excludedResource {
				continue
			}
			expected[request.TypeUrl] = &discovery.DeltaDiscoveryResponse{
		...
		tc := testCase{
			desc: desc.desc,
			inAdsc: &DeltaADSC{
				Received:        make(map[string]*discovery.DeltaDiscoveryResponse),
				Updates:         make(chan string),
				DeltaXDSUpdates: make(chan *discovery.DeltaDiscoveryResponse),
				cfg: &Config{
					InitialDeltaDiscoveryRequests: desc.deltaInitialRequests,
				},
				VersionInfo: map[string]string{},
				sync:        map[string]time.Time{},
			},
			deltaStreamHandler: func(stream discovery.AggregatedDiscoveryService_DeltaAggregatedResourcesServer) error {
				for _, resource := range expected {
					_ = stream.Send(&discovery.DeltaDiscoveryResponse{
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			DeltaStreamHandler = tt.deltaStreamHandler
			...
			xds := grpc.NewServer()
			discovery.RegisterAggregatedDiscoveryServiceServer(xds, new(testDeltaAdscRunServer))
			go func() {
				err = xds.Serve(l)
				if err != nil {
					log.Println(err)
...
func TestDeltaADSC_Save(t *testing.T) {
	tests := []struct {
		desc         string
...
func TestDeltaADSC_handleMCP(t *testing.T) {
	rev := "test-rev"
	adsc := &DeltaADSC{
		VersionInfo: map[string]string{},
