			...
			ctx.NewSubTest("CDS").
				...
				RunParallel(func(ctx framework.TestContext) {
					...
					echoboot.NewBuilderOrFail(ctx, ctx).
						With(&a, localityEchoConfig(ns, "a")).
						With(&b, localityEchoConfig(ns, "b")).
						With(&c, localityEchoConfig(ns, "c")).
						BuildOrFail(ctx)
...
func localityEchoConfig(ns namespace.Instance, name string) echo.Config {
	return echo.Config{
		Service:   name,
		Namespace: ns,
...
func sendLocalityTraffic(from echo.Instance, host string) error {
	headers := http.Header{}
