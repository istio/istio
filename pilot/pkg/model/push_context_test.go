...
func TestUpdateSidecarScope(t *testing.T) {
	...
	ps2 := NewPushContext()
	ps2.Mesh = env.Mesh()
	ps2.ServiceIndex = ps.ServiceIndex
	...
	sidecarWithWorkloadSelectorUpdated := &networking.Sidecar{
		WorkloadSelector: &networking.WorkloadSelector{
			Labels: map[string]string{"app": "foo", "version": "v2"},
		},
		Egress: []*networking.IstioEgressListener{
			{
				Hosts: []string{"default/*"},
	...
	configWithWorkloadSelectorUpdated := config.Config{
		Meta: config.Meta{
			GroupVersionKind: collections.IstioNetworkingV1Alpha3Sidecars.Resource().GroupVersionKind(),
			Name:             "foo",
			Namespace:        "default",
		},
		Spec: sidecarWithWorkloadSelectorUpdated,
	}
	configWithoutWorkloadSelector := config.Config{
		Meta: config.Meta{
			GroupVersionKind: collections.IstioNetworkingV1Alpha3Sidecars.Resource().GroupVersionKind(),
			Name:             "default",
			Namespace:        "default",
	...
	initialSidecars := []config.Config{
		configWithWorkloadSelector,
		rootConfig,
	}

	finalSidecars := []config.Config{
		configWithWorkloadSelectorUpdated,
		configWithoutWorkloadSelector,
		rootConfig2,
	}
	...
	cases := []struct {
		...
		finalDescribe   string
	}{
		{
			proxy:           &Proxy{Type: SidecarProxy, ConfigNamespace: "default"},
			labels:          labels.Instance{"app": "foo", "version": "v1"},
			initialSidecar:  "default/foo",
			finalSidecar:    "default/default",
			initialDescribe: "match namespace sidecar with selector",
			finalDescribe:   "namespace sidecar without selector",
		},
		{
			proxy:           &Proxy{Type: SidecarProxy, ConfigNamespace: "default"},
			labels:          labels.Instance{"app": "foo", "version": "v2"},
			initialSidecar:  "default/global",
			finalSidecar:    "default/foo",
			initialDescribe: "no match namespace sidecar",
			finalDescribe:   "match namespace sidecar with selector",
		},
		{
			proxy:           &Proxy{Type: SidecarProxy, ConfigNamespace: "default"},
			labels:          labels.Instance{"app": "bar"},
			...
			finalSidecar:    "default/default",
			initialDescribe: "no match namespace sidecar",
			finalDescribe:   "matches namespace sidecar without selector",
		},
		{
			proxy:           &Proxy{Type: SidecarProxy, ConfigNamespace: "nosidecar"},
			labels:          labels.Instance{"app": "bar"},
			initialSidecar:  "nosidecar/global",
			finalSidecar:    "nosidecar/global2",
			initialDescribe: "uses global sidecar",
			finalDescribe:   "uses new global sidecar",
		},
		{
			proxy:           &Proxy{Type: Router, ConfigNamespace: "istio-system"},
			labels:          labels.Instance{"app": "istio-gateway"},
			initialSidecar:  "istio-system/default-sidecar",
			finalSidecar:    "istio-system/default-sidecar",
			initialDescribe: "gateway sidecar scope",
			finalDescribe:   "gateway sidecar scope",
		},
	}

	// create initial sidecar configs
	for _, v := range initialSidecars {
		configStore.Create(v)
	}

	// initialize all sidecars
	if err := ps.initSidecarScopes(env); err != nil {
		t.Fatalf("init sidecar scope failed: %v", err)
	}

	// test for initial siecar configs
	for _, c := range cases {
		t.Run(c.initialDescribe, func(t *testing.T) {
			scope := ps.getSidecarScope(c.proxy, c.labels)
			if c.initialSidecar != scopeToSidecar(scope) {
				t.Errorf("should get sidecar %s but got %s", c.initialSidecar, scopeToSidecar(scope))
			}
		})
	}

	// detect changed sidecars
	changedSidecars := make([]ConfigKey, 0)
	for _, f := range finalSidecars {
		found := false
		for _, i := range initialSidecars {
			if f.Name == i.Name && f.Namespace == i.Namespace {
				found = true
			}
		}

		if found {
			configStore.Update(f)
		} else {
			configStore.Create(f)
		}
		changedSidecars = append(changedSidecars, ConfigKey{
			Kind:      kind.Sidecar,
			Name:      f.Name,
	...
	for _, i := range initialSidecars {
		...
		if !found {
			configStore.Delete(i.GroupVersionKind, i.Name, i.Namespace, nil)
			changedSidecars = append(changedSidecars, ConfigKey{
				Kind:      kind.Sidecar,
				Name:      i.Name,
	...
	// test that sidecar configs are updated to their final desired state
	for _, c := range cases {
		t.Run(c.finalDescribe, func(t *testing.T) {
			scope := ps2.getSidecarScope(c.proxy, c.labels)
			if c.finalSidecar != scopeToSidecar(scope) {
				t.Errorf("should get sidecar %s but got %s", c.finalSidecar, scopeToSidecar(scope))
			}
		})
	}
}
