...
type (
	...
	Metric interface {
		...
		// this is equivalent to subtracting 1 to the current value.
		Decrement()
...
func MustRegisterViews(metrics ...Metric) {
	for _, m := range metrics {
		if err := view.Register(m.View()); err != nil {
			panic(err)
		}
	}
}

func UnregisterViews(metrics ...Metric) {
	for _, m := range metrics {
		view.Unregister(m.View())
	}
}
...
// NewCount creates a new Metric with an aggregation type of Count. That means that data collected
// by the new Metric will export only the count value.
// For example, total number of accepted requests can be aggregated by using Count.
func NewCount(name, description string, tags ...Tag) Metric {
	return newMetric(name, description, view.Count(), tags...)
}
...
func (f *float64Metric) Record(value float64) {
	stats.RecordWithTags(context.Background(), f.tagValues, f.M(value)) //nolint:errcheck
}

func (f *float64Metric) WithTags(tagValues ...TagValue) Metric {
	t := make([]tag.Mutator, len(f.tagValues))
	copy(t, f.tagValues)
