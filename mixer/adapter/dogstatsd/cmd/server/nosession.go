...
import (
	...
	config "istio.io/istio/mixer/adapter/dogstatsd/config"
...
// NewDogstatsdNoSessionServer creates a new no session server based on given args.
func NewDogstatsdNoSessionServer(addr uint16, poolSize int, c *Cert) (*NoSession, error) {
	saddr := fmt.Sprintf(":%d", addr)

	gp := pool.NewGoroutinePool(poolSize, false)
	inf := dogstatsd.GetInfo()
	s := &NoSession{
		builder:    inf.NewBuilder(),
		env:        handler.NewEnv(0, "dogstatsd-nosession", gp),
