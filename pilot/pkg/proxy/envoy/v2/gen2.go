...
// watchedResource finds the per-connection info associated with the TypeURL.
// The server keeps track of version, nonce and other info about the TypeURL.
func (s *DiscoveryServer) watchedResource(con *XdsConnection, discReq *discovery.DiscoveryRequest) (w *model.WatchedResource, isNew bool) {
	isNew = false
...
func (s *DiscoveryServer) handleAck(con *XdsConnection, w *model.WatchedResource) bool {
	...
	discReq := w.LastRequest

	// TODO: all acks should return ErrorDetail with status 0 !
