...
func (m Model) Generate(forTCP bool, forGRPC bool, action rbacpb.RBAC_Action) (*rbacpb.Policy, error) {
	...
	for _, rl := range m.principals {
		principal, err := generatePrincipal(rl, forTCP, forGRPC, action)
		if err != nil {
			return nil, err
		}
...
func generatePrincipal(rl ruleList, forTCP bool, forGRPC bool, action rbacpb.RBAC_Action) (*rbacpb.Principal, error) {
	var and []*rbacpb.Principal
	for _, r := range rl.rules {
		ret, err := r.principal(forTCP, forGRPC, action)
		if err != nil {
			return nil, err
		}
		and = append(and, ret...)
...
func (r rule) principal(forTCP bool, forGRPC bool, action rbacpb.RBAC_Action) ([]*rbacpb.Principal, error) {
	var principals []*rbacpb.Principal
	...
	or = nil
	for _, notValue := range r.notValues {
		p, err := r.g.principal(r.key, notValue, forTCP, forGRPC)
		if err := r.checkError(action, err); err != nil {
			return nil, err
		}
		if p != nil {
