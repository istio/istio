...
func TestStatusMaps(t *testing.T) {
	...
	Expect(r.reverseStatus).To(Equal(map[string]sets.Set[string]{"a": {"conA~": x, "conB~": x}, "c": {"conC~": x}, "d": {"conD~": x}}))
	r.processEvent("conA", typ, "d")
	Expect(r.status).To(Equal(map[string]string{"conA~": "d", "conB~": "a", "conC~": "c", "conD~": "d"}))
	Expect(r.reverseStatus).To(Equal(map[string]sets.Set[string]{"a": {"conB~": x}, "c": {"conC~": x}, "d": {"conD~": x, "conA~": x}}))
	r.RegisterDisconnect("conA", []xds.EventType{typ})
	Expect(r.status).To(Equal(map[string]string{"conB~": "a", "conC~": "c", "conD~": "d"}))
	Expect(r.reverseStatus).To(Equal(map[string]sets.Set[string]{"a": {"conB~": x}, "c": {"conC~": x}, "d": {"conD~": x}}))
}
...
func initReporterWithoutStarting() (out Reporter) {
	...
	out.cm = nil // TODO
	out.reverseStatus = make(map[string]sets.Set[string])
