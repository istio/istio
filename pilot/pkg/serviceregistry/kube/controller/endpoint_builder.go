...
type EndpointBuilder struct {
	...
	nodeName string
	// Whether the pod's traffic is redirected to ztunnel.
...
func (c *Controller) NewEndpointBuilder(pod *v1.Pod) *EndpointBuilder {
	...
	if pod != nil {
		...
		// The CNI sets this on every captured pod, whether ambient was enabled on the pod or on
		// its namespace, so it is the only signal that holds for both.
		ambientCaptured = pod.Annotations[annotation.AmbientRedirection.Name] == constants.AmbientRedirectionEnabled
	}
	...
	out := &EndpointBuilder{
		...
		labels:          podLabels,
		nodeName:        node,
		ambientCaptured: ambientCaptured,
	}
...
func (b *EndpointBuilder) buildIstioEndpoint(
	...
	return &model.IstioEndpoint{
		...
		NodeName:               b.nodeName,
		AmbientCaptured:        b.ambientCaptured,
	}
}
