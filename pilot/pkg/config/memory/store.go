...
func Make(schemas collection.Schemas) *Store {
	return newStore(schemas, false, nil, nil, nil)
}
...
func MakeSkipValidation(schemas collection.Schemas) *Store {
	return newStore(schemas, true, nil, nil, nil)
}
...
func newStore(
	...
	opts := krt.NewOptionsBuilder(stop, "memory", debugger)
	out := Store{
		schemas:        schemas,
	...
	for _, s := range schemas.All() {
		collection := krt.NewStaticCollection[config.Config](syncer, nil, opts.WithName(s.Kind())...)
