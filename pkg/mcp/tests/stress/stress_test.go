...
func BenchmarkSteadyState_1Client_100Types(b *testing.B) {
	benchmarkSteadyState(1, 100, b)
}

func BenchmarkSteadyState_10Clients_10Types(b *testing.B) {
	benchmarkSteadyState(10, 10, b)
}

func BenchmarkSteadyState_100Clients_100Types(b *testing.B) {
	benchmarkSteadyState(10, 100, b)
}

func benchmarkSteadyState(clients, collections int, b *testing.B) {
	opts := defaultOptions()
...
func benchmarkUnknownType(clients int, b *testing.B) {
	fmt.Printf("Beginning GoRoutines: %d\n", runtime.NumGoroutine())

	opts := defaultOptions()
	opts.clients = clients
	opts.setupFn = func(d *driver) {
		d.clientOpts.CollectionOptions = sink.CollectionOptionsFromSlice(generateCollectionNames(d.o.collections, 1))
	}
	d, err := newDriver(opts)
	if err != nil {
		b.Fatalf("error creating driver: %v", err)
	...
	fmt.Printf("Before run GoRoutines: %d\n", runtime.NumGoroutine())
	d.run(b)
	fmt.Printf("After run GoRoutines: %d\n", runtime.NumGoroutine())

	d.Close()
	time.Sleep(time.Millisecond*500)
	fmt.Printf("After close GoRoutines: %d\n", runtime.NumGoroutine())
	fmt.Printf("STEPS: %d\n", d.steps)
...
func newDriver(o options) (*driver, error) {
	...
	lo.SetOutputLevel("mcp", log.NoneLevel)
	lo.SetOutputLevel("default", log.NoneLevel)
	log.Configure(lo)
...
func (d *driver) initDataset() {
	d.collections = generateCollectionNames(d.o.collections, 0)
	...
	for i := 1; i < NO_DATASETS; i++ { // iterate over data sets
		b := prev.Builder()
		for _, t := range d.collections { // iterate over type urls
...
func (d *driver) initOptions() {
	d.serverOpts = &source.Options{
		...
		CollectionsOptions: source.CollectionOptionsFromSlice(d.collections),
	...
	d.clientOpts = &sink.Options{
		ID:                "driver",
		Updater:           d,
		CollectionOptions: sink.CollectionOptionsFromSlice(d.collections),
		Reporter:          monitoring.NewInMemoryStatsContext(),
	}
}
