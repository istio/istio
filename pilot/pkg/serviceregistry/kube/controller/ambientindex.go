	...
	if isDelete {
		for _, wl := range a.byPod {
			...
			if filtered {
				...
				updates.Insert(model.ConfigKey{Kind: kind.Address, Name: wl.ResourceName(), Namespace: wl.Network})
			}
...
func (c *Controller) AuthorizationPolicyHandler(old config.Config, obj config.Config, ev model.Event) {
	...
	for ip, pod := range pods {
		...
		updates[model.ConfigKey{Kind: kind.Address, Name: newWl.ResourceName(), Namespace: newWl.Network}] = struct{}{}
	}
...
func (a *AmbientIndex) handlePod(oldObj, newObj any, isDelete bool, c *Controller) sets.Set[model.ConfigKey] {
	...
	if wl == nil {
		...
		if oldWl != nil {
			...
			updates.Insert(model.ConfigKey{Kind: kind.Address, Name: p.Status.PodIP, Namespace: oldWl.Network})
			return updates
		}
	...
	updates.Insert(model.ConfigKey{Kind: kind.Address, Name: p.Status.PodIP, Namespace: wl.Network})
	return updates
}
...
func (a *AmbientIndex) handleService(obj any, isDelete bool, c *Controller) map[model.ConfigKey]struct{} {
	...
	for _, vip := range vips {
		for _, wl := range a.byService[vip] {
			updates[model.ConfigKey{Kind: kind.Address, Name: wl.ResourceName(), Namespace: wl.Network}] = struct{}{}
		}
	}
	// Update indexes
	if isDelete {
...
func (c *Controller) WorkloadInfos(addresses sets.Set[types.NamespacedName]) ([]*model.WorkloadInfo, []string) {
	...
	for p := range addresses {
		...
		// TODO: filter out workloads that are in different networks.
		if len(wl) == 0 {
...
func (c *Controller) constructWorkload(pod *v1.Pod, waypoints []string, policies []string) *workloadapi.Workload {
	...
	wl := &workloadapi.Workload{
		...
		Network:               string(c.Network(pod.Status.PodIP, pod.Labels)),
