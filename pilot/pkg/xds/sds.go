...
func parseResourceName(resource, defaultNamespace string, cluster cluster.ID) (SecretResource, error) {
	sep := "/"
...
func parseResources(names []string, proxy *model.Proxy) []SecretResource {
	...
	for _, resource := range names {
		sr, err := parseResourceName(resource, proxy.VerifiedIdentity.Namespace, proxy.Metadata.ClusterID)
		if err != nil {
