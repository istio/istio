...
test.integration.%.kube: | $(JUNIT_REPORT) check-go-tag
	...
	--test.run=TestRequestAuthentication \
	2>&1 | tee >($(JUNIT_REPORT) > $(JUNIT_OUT))
...
test.integration.kube.presubmit: | $(JUNIT_REPORT) check-go-tag
	PATH=${PATH}:${ISTIO_OUT} $(GO) test -p 1 -vet=off ${T} -tags=integ $(shell go list -tags=integ ./tests/integration/... | grep -v /qualification | grep -v /examples) -timeout 30m \
	${_INTEGRATION_TEST_FLAGS} ${_INTEGRATION_TEST_SELECT_FLAGS} \
	2>&1 | tee >($(JUNIT_REPORT) > $(JUNIT_OUT))
