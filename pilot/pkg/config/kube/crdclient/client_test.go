...
func makeClientVersions(t *testing.T, schemas collection.Schemas, versions map[string]struct{}) (model.ConfigStoreCache, kube.ExtendedClient) {
	fake := kube.NewFakeClient()
	for _, s := range schemas.All() {
...
func runCRUDTest(t *testing.T, c collection.Schema, store model.ConfigStoreCache) {
	configName := "name"
...
func createCRD(t test.Failer, client kube.Client, r resource.Schema, versions map[string]struct{}) {
	t.Helper()
	var vers []v1.CustomResourceDefinitionVersion
	if len(versions) == 0 {
		versions = map[string]struct{}{r.GroupVersionKind().Version: {}}
	}
...
func TestClientClusterVersion(t *testing.T) {
	...
	for _, versions := range versions {
		for _, res := range schema.All() {
			t.Run(res.Resource().Kind()+"_"+strings.Join(UnsortedList(versions), "-"), func(t *testing.T) {
...
func UnsortedList(s map[string]struct{}) []string {
