...
func (i *index) Insert(wi *model.WorkloadInstance) *model.WorkloadInstance {
	...
	if previous != nil && previous.Endpoint.GetIstioEndpointKey() != wi.Endpoint.GetIstioEndpointKey() {
		i.ipToKeys.Delete(previous.Endpoint.GetIstioEndpointKey(), key)
	}
	i.keyToInstance[key] = wi
	if len(wi.Endpoint.Addresses) > 0 {
		i.ipToKeys.Insert(wi.Endpoint.GetIstioEndpointKey(), key)
	}
	return previous
}
...
func (i *index) Delete(wi *model.WorkloadInstance) *model.WorkloadInstance {
	...
	i.ipToKeys.Delete(wi.Endpoint.GetIstioEndpointKey(), key)
