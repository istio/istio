...
func hostCases(t TrafficContext) {
	for _, c := range t.Apps.A {
		...
		hosts := []string{
			...
			fmt.Sprintf("%s.%s:%d", cfg.Service, cfg.Namespace.Name(), port),
			fmt.Sprintf("%s.%s.svc:%d", cfg.Service, cfg.Namespace.Name(), port),
			fmt.Sprintf("%s.%s:12345", cfg.Service, cfg.Namespace.Name()),
			...
			fmt.Sprintf("some-instances.%s.%s", cfg.Service, cfg.Namespace.Name()),
			fmt.Sprintf("some-instances.%s.%s.svc", cfg.Service, cfg.Namespace.Name()),
			fmt.Sprintf("some-instances.%s.%s:12345", cfg.Service, cfg.Namespace.Name()),
		...
		hosts = []string{
			...
			fmt.Sprintf("some-instances.%s.%s", cfg.Service, cfg.Namespace.Name()),
			fmt.Sprintf("some-instances.%s.%s.svc", cfg.Service, cfg.Namespace.Name()),
			fmt.Sprintf("some-instances.%s.%s:%d", cfg.Service, cfg.Namespace.Name(), port),
