...
func setAutoSniAndAutoSanValidation(mc *clusterWrapper, tls *networking.ClientTLSSettings) {
	...
	// For DFP clusters, set auto_san_validation if tls.Mode is not ISTIO_MUTUAL as these certs do not have DNS SANs required for auto_san_validation.
	// SIMPLE_TLS and MUTUAL_TLS certs need to have DNS SANs that match hostname used for the connection for auto_san_validation to work.
	// For non-DFP clusters, require auto_san_validation if auto_sni is set and there are no explicit SANs
	if ((mc.isDFPCluster && tls.Mode != networking.ClientTLSSettings_ISTIO_MUTUAL) ||
