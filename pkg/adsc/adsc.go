...
type ADSC struct {
	...
	edsClusters map[string]*clusterv3.Cluster
	...
	eds map[string]*endpointv3.ClusterLoadAssignment
...
func (a *ADSC) handleCDS(ll []*clusterv3.Cluster) {

	cn := []string{}
	cdsSize := 0
	edscds := map[string]*clusterv3.Cluster{}
	cds := map[string]*clusterv3.Cluster{}
	for _, c := range ll {
...
func (a *ADSC) handleEDS(eds []*endpointv3.ClusterLoadAssignment) {
	la := map[string]*endpointv3.ClusterLoadAssignment{}
	edsSize := 0
	ep := 0
	for _, cla := range eds {
...
func (a *ADSC) GetEdsClusters() map[string]*clusterv3.Cluster {
	a.mutex.Lock()
	defer a.mutex.Unlock()
...
func (a *ADSC) GetClusters() map[string]*clusterv3.Cluster {
	a.mutex.Lock()
	defer a.mutex.Unlock()
	return a.clusters
}
...
func (a *ADSC) GetEndpoints() map[string]*endpointv3.ClusterLoadAssignment {
	a.mutex.Lock()
	defer a.mutex.Unlock()
	return a.eds
}
