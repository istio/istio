...
func detectSigningCABundle() (ca.SigningCAFileBundle, error) {
	...
	// If tls.crt is used - the cert may include intermediaries, but not the root cert(s)
	//
	if _, err := os.Stat(tlsSigningFile); err == nil {
		log.Infof("Using kubernetes.io/tls secret type for signing ca files %s", tlsSigningFile)
		return ca.SigningCAFileBundle{
			RootCertFile: path.Join(LocalCertDir.Get(), ca.TLSSecretRootCertFile),
			// This is empty when tls.crt is used - intermediary certs are already included in SigningCertFile.
			// With 'old style' cacerts, the SigningCertFile includes only the leaf, and the intermediaries are in
			// this field.
			CertChainFiles: nil,
			// tls.crt is concatenation of leaf certificate and all intermediary certs - but not root.
			// The root used to sign the last element in the chain ( top intermediary ) is included in the ca.crt bundle,
			// which may have other trusted roots.
	...
	// TODO: check if CertChainFiles exist, check if RootCertFile exists - and make the selection more deterministic.
	return ca.SigningCAFileBundle{
...
func (s *Server) createIstioCA(opts *caOptions) (*ca.IstioCA, error) {
	...
	// The CA cert detected in `cacerts` was generated by Istiod and not plugged in by user.
	var istioGenerated bool
	var err error

	// For mounted certificates (if any) determine the set of files.
	// If not found - we'll load from K8S Secret.
	// This will return the 'new style' tls.crt file paths or old style - but they may not exist.
	fileBundle, err := detectSigningCABundle()
	if err != nil {
		return nil, fmt.Errorf("unable to determine signing file format %v", err)
	}
	// the fileBundle will have file paths - but they may not exist.
	...
	// TODO: if the cert chain length > 1 - we can't refresh so we can use the other
	// code path, and ignore detectedSigningCABundle file (which really triggers issuing
	// refresh certs). We can use the length of the chain instead of 'istioGenerated' label.

	if !detectedSigningCABundle || (features.UseCacertsForSelfSignedCA && istioGenerated) {
		// No mounted certs, or mounted certs but istio-generated (UseCacertsForSelfSignedCA or marker)
		//
		// To simplify - don't mount, use Secret code path in all cases.
		...
		caOpts, err = s.loadCAFromSecrets(&fileBundle, opts)
		if err != nil {
			return nil, err
		}
		...
		log.Infof("Use local CA certificate from %s, not auto-generated by Istiod", fileBundle.RootCertFile)
...
// loadCAFromSecrets will initialize the Istio CA using istio-ca-secrets or cacerts.
// This is only called if mounted files are not found
func (s *Server) loadCAFromSecrets(fileBundle *ca.SigningCAFileBundle, opts *caOptions) (*ca.IstioCAOptions, error) {
	var caOpts *ca.IstioCAOptions
	var err error
	if s.kubeClient != nil {
		log.Info("Use in-cluster Secret  as the CA certificate")
...
// ca cert can come from two sources, order matters:
...
// 2. Extract from the cert-chain signed by other CSR signer.
func (s *Server) createIstioRA(opts *caOptions) (ra.RegistrationAuthority, error) {
	...
	if err != nil {
		...
		// File does not exist.
		// TODO(costin): panic instead ?

		if certSignerDomain == "" {
			// We have no way to find the roots - this code path is not using MeshConfig.
		...
		if certSignerDomain == "" {
			...
			log.Infof("K8S RA with domain %s using roots from %s, MeshConfig roots ignored", certSignerDomain, caCertFile)
		}
