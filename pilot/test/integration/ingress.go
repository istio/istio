...
func (t *ingress) run() error {
	...
	cases := []struct {
		...
		{"a", fmt.Sprintf("http://%s.%s/javastyle/myservlet", ingressServiceName, t.IstioNamespace), ""},
		{"a", fmt.Sprintf("http://%s.%s/.well_known/foo", ingressServiceName, t.IstioNamespace), ""},
		{"a", fmt.Sprintf("http://%s.%s/.well_known/foo/prefix/bar", ingressServiceName, t.IstioNamespace), ""},
