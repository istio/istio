...
func (k *KubeInfo) Teardown() error {
	...
	if *installer == helmInstallerName {
		// clean up using helm
		err := util.HelmDelete(istioHelmChartlName)
		if err != nil {
			return nil
		}
...
func (k *KubeInfo) deployIstioWithHelm() error {
	...
	err = util.HelmInstallDryRun(workDir, istioHelmChartlName, valFile, k.Namespace, setValue)
	if err != nil {
	...
	// helm install
	err = util.HelmInstall(workDir, istioHelmChartlName, valFile, k.Namespace, setValue)
	if err != nil {
