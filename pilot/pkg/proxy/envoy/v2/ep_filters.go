...
func EndpointsByNetworkFilter(endpoints []endpoint.LocalityLbEndpoints, conn *XdsConnection, env *model.Environment) []endpoint.LocalityLbEndpoints {
	...
	for _, ep := range endpoints {
		...
		for n, w := range remoteEps {
			...
			if !found {
				adsLog.Infof("the endpoints within network %d will be ignored for no network configured", n)
				continue
			}
			gws := networkConf.Gateways
			if len(gws) == 0 {
				adsLog.Infof("the endpoints within network %d will be ignored for no gateways configured", n)
				continue
			}
