...
func TestController(t *testing.T) {
	framework.
		...
		Run(func(ctx framework.TestContext) {
			...
			installWithCRFile(t, ctx, cs, s, istioCtl, "demo-test", "demo", "", nil)
			installWithCRFile(t, ctx, cs, s, istioCtl, "default-test", "default", "", nil)
			...
			istioCtl.InvokeOrFail(t, initCmd)
			installWithCRFile(t, ctx, cs, s, istioCtl, "minimal-test", "minimal", "v2", nil)
			// apply separate gateways
			installWithCRFile(t, ctx, cs, s, istioCtl, "gateways-test", "empty", "v2", []string{"values.gateways.istio-ingressgateway.enabled=true"})
			// verify again to make sure istiod and gateways are not pruned
			verifyInstallation(t, ctx, istioCtl, "minimal-test", "minimal", "v2", nil, cs)
...
func checkInstallStatus(cs istioKube.ExtendedClient, crName, revision string) error {
	...
	retryFunc := func() error {
		us, err := cs.Dynamic().Resource(gvr).Namespace(IstioNamespace).Get(context.TODO(), revName(crName, revision), kubeApiMeta.GetOptions{})
		if err != nil {
...
func installWithCRFile(t *testing.T, ctx resource.Context, cs cluster.Cluster, s *image.Settings,
	istioCtl istioctl.Instance, crName, profileName, revision string, setFlags []string) {
...
`
	...
	fileOverlayYAML := fmt.Sprintf(metadataYAML, revName(crName, revision), profileName, ManifestPathContainer, s.Hub, s.Tag, s.PullPolicy)

	// Combine file and --set overlays to IOP format.
	overlayYAML, err := manifest.OverlaySetFlagValues(fileOverlayYAML, setFlags)
	if err != nil {
		t.Fatalf("failed to overlay iop cr with setFlags: %v", err)
	}
...
func verifyInstallation(t *testing.T, ctx resource.Context, istioCtl istioctl.Instance,
	crName, profileName, revision string, setFlags []string, cs cluster.Cluster) object.K8sObjects {
	scopes.Framework.Infof("=== verifying istio installation revision %s === ", revision)
	if err := checkInstallStatus(cs, crName, revision); err != nil {
	...
	for _, sf := range setFlags {
		generateCmd = append(generateCmd, "--set", sf)
	}
