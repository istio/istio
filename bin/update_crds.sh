...
if [ ! -f "${API_TMP}/kubernetes/standard.gen.yaml" ]; then
  echo "Generated Custom Resource Definitions file for Standard Release Channel does not exist in the commit SHA ${SHA}. Not updating the CRD file."
  exit
fi
rm -f "${ROOTDIR}/manifests/charts/base/files/standard.gen.yaml"
cp "${API_TMP}/kubernetes/standard.gen.yaml" "${ROOTDIR}/manifests/charts/base/files/standard.gen.yaml"
