...
const (
	...
	TLSSecretOcspStaple = "ocsp_staple.der"
)
...
func (s *CredentialsController) GetKeyAndCert(name, namespace string) (key []byte, cert []byte, staple []byte, err error) {
...
func extractKeyAndCert(scrt *v1.Secret) (key, cert, staple []byte, err error) {
	...
	if hasKeys(scrt.Data, TLSSecretCert, TLSSecretKey, TLSSecretOcspStaple) {
		return nil, nil, nil, fmt.Errorf("found keys %q and %q and %q, but they were empty", TLSSecretCert, TLSSecretKey, TLSSecretOcspStaple)
	}
