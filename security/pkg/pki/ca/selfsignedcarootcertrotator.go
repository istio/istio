...
// rotates or reload secret-based (self-signed or external) root cert periodically.
// The code for handling the /etc/cacerts is separate.
// nolint: gosec
...
func (rotator *SelfSignedCARootCertRotator) Run(stopCh chan struct{}) {
	if rotator.config.enableJitter {
		rootCertRotatorLog.Debugf("Jitter is enabled, wait %s before "+
			"starting root cert rotator.", rotator.backOffTime.String())
		select {
		case <-time.After(rotator.backOffTime):
			rootCertRotatorLog.Debugf("Jitter complete, start rotator.")
		case <-stopCh:
			rootCertRotatorLog.Debug("Received stop signal, so stop the root cert rotator.")
			return
		}
	}
	...
	for {
		select {
		case <-ticker.C:
			rootCertRotatorLog.Debug("Check and rotate root cert.")
...
func (rotator *SelfSignedCARootCertRotator) checkAndRotateRootCertForSigningCertCitadel(
	...
	tlsCrt := caSecret.Data[TLSSecretCACertFile]
	if tlsCrt != nil {
		// This is a 'tls' style secret, we can't refresh it (not generated by us).
		return
	}

	// Check root certificate expiration time in CA secret - this is using the self-generated secret.
	...
	if err == nil && waitTime > 0 {
		...
		// cert bundle, it implies that other Citadels or the user has updated istio-ca-secret or cacerts.
