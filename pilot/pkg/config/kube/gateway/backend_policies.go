...
func BackendTrafficPolicyCollection(
	...
	return krt.NewStatusManyCollection(trafficPolicies, func(ctx krt.HandlerContext, i *gatewayx.XBackendTrafficPolicy) (
		*gatewayx.PolicyStatus,
		...
		// TODO: implement i.Spec.SessionPersistence. This will need to map into a StatefulSession filter which Istio doesn't
		// currently support on DestinationRule
		if i.Spec.SessionPersistence != nil {
			unsupported = append(unsupported, "sessionPersistence")
		}
		// TODO: implement i.Spec.RetryConstraint. This doesn't have upstream Envoy support
		if i.Spec.RetryConstraint != nil {
			unsupported = append(unsupported, "retryConstraint")
		}
		if len(unsupported) > 0 {
			conds[string(gatewayalpha2.PolicyConditionAccepted)].message = fmt.Sprintf("Configuration is valid, but Istio does not support the following fields: %v", humanReadableJoin(unsupported))
		}
