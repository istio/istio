...
func TestController_HostInstances(t *testing.T) {
	...
	pods := []*v1.Pod{
		generatePod("pod1", "ns", sa, "node1", map[string]string{"app": "test-app"}),
		generatePod("pod2", "ns", sa, "node1", map[string]string{"app": "prod-app"}),
		generatePod("pod3", "ns", sa, "node2", map[string]string{"app": "prod-app"}),
	}
	// Add port alias annotation for pod2. This will ovewrite service level annotation (if any)
	pods[1].Annotations = map[string]string{portAliasAnnotationKey(8080): "9999"}
	addPods(t, controller, pods...)
	...
	// Populate pod cache.
	controller.pods.keys["128.0.0.1"] = "ns/pod1"
	controller.pods.keys["128.0.0.2"] = "ns/pod2"
	controller.pods.keys["128.0.0.3"] = "ns/pod3"

	createService(controller, "svc1", "ns",
		map[string]string{portAliasAnnotationKey(8080): "88"},
		[]int32{8080}, map[string]string{"app": "prod-app"}, t)
	createService(controller, "svc2", "ns", nil, []int32{8080}, map[string]string{"app": "staging-app"}, t)

	// Endpoints are generated by Kubernetes from pod labels and service selectors.
	// Here we manually create them for mocking purpose.
	svc1Ips := []string{"128.0.0.1", "128.0.0.2"}
	svc2Ips := []string{"128.0.0.3"}
	portNames := []string{"test-port"}
	createEndpoints(controller, "svc1", "ns", portNames, svc1Ips, t)
	createEndpoints(controller, "svc2", "ns", portNames, svc2Ips, t)
	...
	for _, test := range testCases {
		hostInstances, err := controller.HostInstances(map[string]bool{test.addrs: true})
		if err != nil {
			t.Error("Error getting host instances: ", err)
		}
		if len(hostInstances) != 1 {
			t.Errorf("Exepect 1 hostInstances, get %d", len(hostInstances))
			continue
		}
		if hostInstances[0].Endpoint.Port != test.wantedPort || hostInstances[0].Endpoint.PortAlias != test.wantedAliasPort {
			t.Errorf("Wanted host instance with port = %d, and alias port = %d, get %d and %d",
				test.wantedPort, test.wantedAliasPort,
				hostInstances[0].Endpoint.Port, hostInstances[0].Endpoint.PortAlias)
		}
	}
}
...
func createEndpoints(controller *Controller, name, namespace string, portNames, ips []string, t *testing.T) {
	...
	for _, name := range portNames {
		eps = append(eps, v1.EndpointPort{Name: name, Port: 8080})
	}
