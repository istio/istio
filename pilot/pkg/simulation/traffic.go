...
func (r Result) Matches(t *testing.T, want Result) {
	...
	// Patch on the `FilterChainMatched` of the result so that a `StrictMatch` would still work
	if r.filterChainAutoGenerated() {
		r.FilterChainMatched = ""
	}
...
func (r Result) filterChainAutoGenerated() bool {
	if r.FilterChainMatched != "" {
		fcName := strings.Split(r.FilterChainMatched, v1alpha3.FilterChainNameInfix)

		if len(fcName) > 1 && fcName[0] == r.ListenerMatched {
			return true
		}
	}
	return false
