...
// modifyPruneResourcesBasedOnK8SVersion modifies the pruning resources based on the k8s version
func (h *HelmReconciler) modifyPruneResourcesBasedOnK8SVersion() {
	// autoscaling v2 API is available on >=1.23
	if kube.IsAtLeastVersion(h.kubeClient, autoscalingV2MinK8SVersion) {
		NamespacedResources = append(NamespacedResources, autoScalingV2GVK)
	}
}
