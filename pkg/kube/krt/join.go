...
func JoinWithMergeCollection[T any](cs []Collection[T], merge func(ts []T) *T, opts ...CollectionOption) Collection[T] {
	// For merging, leverage the mergejoin infrastructure via NestedJoinWithMergeCollection
	// by wrapping the static list of collections in a Collection[Collection[T]]
	o := buildCollectionOptions(opts...)
	if o.name == "" {
		o.name = fmt.Sprintf("Join[%v]", ptr.TypeName[T]())
	}

	// Create a syncer that waits for all input collections to sync
	synced := make(chan struct{})
	go func() {
		for _, c := range cs {
			if !c.WaitUntilSynced(o.stop) {
	...
	// Wrap the collections in a static collection so we can use NestedJoinWithMergeCollection
	staticOpts := append(opts, WithName(o.name+"-static"))
	staticCollections := NewStaticCollection(
		channelSyncer{synced: synced},
		cs,
		staticOpts...,
	)

	return NestedJoinWithMergeCollection(staticCollections, merge, opts...)
}
