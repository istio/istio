...
type networkManager struct {
	...
	gatewayResourceClient kclient.Informer[*k8sv1.Gateway]
...
func initNetworkManager(c *Controller, options Options) *networkManager {
	...
	if features.MultiNetworkGatewayAPI || features.EnableAmbientControllers {
		n.gatewayResourceClient = kclient.NewDelayedInformer[*k8sv1.Gateway](c.client, gvr.KubernetesGateway, kubetypes.StandardInformer, kubetypes.Filter{})
	}
...
func (n *networkManager) handleGatewayResource(_ *k8sv1.Gateway, gw *k8sv1.Gateway, event model.Event) error {
	...
	autoPassthrough := func(l k8sv1.Listener) bool {
	...
	for _, addr := range gw.Spec.Addresses {
		...
		if addrType := *addr.Type; addrType != k8sv1.IPAddressType && addrType != k8sv1.HostnameAddressType {
			continue
		}
