...
function parse_topology() {
  ...
  NUM_CLUSTERS=$(jq 'length' "${FILE}")
  ITER_END=$((NUM_CLUSTERS-1))

  for i in $(seq 0 $ITER_END); do
    CLUSTER_ITEM=$(jq -r ".[$i]" "${FILE}")
  ...
  export INTEGRATION_TEST_NETWORKS=$(IFS=','; echo "${NETWORK_TOPOLOGIES[*]}")
  export INTEGRATION_TEST_CONTROLPLANE_TOPOLOGY=$(IFS=','; echo "${CONTROLPLANE_TOPOLOGIES[*]}")
  export INTEGRATION_TEST_CONFIG_TOPOLOGY=$(IFS=','; echo "${CONFIG_TOPOLOGIES[*]}")
}


function topology_flags() {
  parse_topology "${1}"
  cat << EOF
--istio.test.kube.controlPlaneTopology=$INTEGRATION_TEST_NETWORKS \\
--istio.test.kube.controlPlaneTopology=$INTEGRATION_TEST_CONTROLPLANE_TOPOLOGY \\
--istio.test.kube.configTopology=INTEGRATION_TEST_CONFIG_TOPOLOGY
EOF
}