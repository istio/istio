...
function loadBaselineFile() {
    local FILENAME="${1}"
    # Filter comment lines then empty lines
    echo "${FILENAME}" \
            | sed -e 's/#.*$//' \
            | sed -e '/^$/d'
}
...
function compareBenchResults() {
    ...
    echo "${BASELINE}" | while read -r BASELINE_ENTRY; do
        local BENCH_NAME
