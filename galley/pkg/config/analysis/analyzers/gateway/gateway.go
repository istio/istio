...
func (s *IngressGatewayPortAnalyzer) Analyze(c analysis.Context) {
	c.ForEach(metadata.IstioNetworkingV1Alpha3Gateways.Name, func(r *resource.Instance) bool {
...
func (*IngressGatewayPortAnalyzer) analyzeGateway(r *resource.Instance, c analysis.Context) {
	...
	c.ForEach(metadata.K8SCoreV1Pods.Name, func(rPod *resource.Instance) bool {
		pod := rPod.Message.(*v1.Pod)
		podLabels := k8s_labels.Set(pod.ObjectMeta.Labels)
		if gwSelector.Matches(podLabels) {
			gwSelectorMatches++
			c.ForEach(metadata.K8SCoreV1Services.Name, func(rSvc *resource.Instance) bool {
	...
	if gwSelectorMatches == 0 {
		...
		if len(gw.Selector) != 1 || gw.Selector["istio"] != "ingressgateway" {
			c.Report(metadata.IstioNetworkingV1Alpha3Gateways.Name, msg.NewReferencedResourceNotFound(r, "selector", gwSelector.String()))
			return
		}
	...
	for _, server := range gw.Servers {
		if server.Port != nil {
			...
			if !ok {
				c.Report(metadata.IstioNetworkingV1Alpha3Gateways.Name, msg.NewGatewayPortNotOnWorkload(r, gwSelector.String(), int(server.Port.Number)))
			}
		}
	}
}
