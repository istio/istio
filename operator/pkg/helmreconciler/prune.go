...
func (h *HelmReconciler) DeleteComponent(componentName string) error {
	...
	for _, gvk := range append(namespacedResources, nonNamespacedResources...) {
		...
		errs = util.AppendErr(errs, h.PruneUnlistedResources(map[string]bool{}, labels, componentName, objects))
	}
	return errs.ToError()
}
...
func (h *HelmReconciler) pruneUnlistedResources(excluded map[string]bool, componentName string) error {
	...
	for _, gvk := range append(namespacedResources, nonNamespacedResources...) {
		...
		for cname, manifest := range manifests.Consolidated() {
			errs = util.AppendErr(errs, h.PruneUnlistedResources(allObjectHashes(manifest), labels, cname, objects))
		}
	}
	return errs.ToError()
}

func (h *HelmReconciler) PruneUnlistedResources(excluded map[string]bool, coreLabels map[string]string, componentName string, objects *unstructured.UnstructuredList) error {
	var errs util.Errors
