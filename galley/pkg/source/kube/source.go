...
var (
	// TODO(nmittler): Pilot allows specifying the namespace for the shared informer.
	namespace = ""
)
...
func New(interfaces client.Interfaces, resyncPeriod time.Duration, schema *schema.Instance,
	...
	for i, spec := range schema.All() {
		...
		if builtin.IsBuiltIn(spec.Kind) {
			...
			sharedInformers = getSharedInformers(sharedInformers, cl, resyncPeriod, namespace)
...
func getSharedInformers(sharedInformers informers.SharedInformerFactory,
	cl kubernetes.Interface, resyncPeriod time.Duration, namespace string) informers.SharedInformerFactory {
	if sharedInformers != nil {
		return sharedInformers
	}
	return informers.NewSharedInformerFactoryWithOptions(cl, resyncPeriod, informers.WithNamespace(namespace))
}
