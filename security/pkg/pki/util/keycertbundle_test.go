...
func TestNewVerifiedKeyCertBundleFromFile(t *testing.T) {
	testCases := map[string]struct {
		...
		"Failure - 3 level CA without cert chain file": {
			...
			certChainFile: "",
			rootCertFile:  rootCertFile,
			expectedErr:   "cannot verify the cert with the provided root chain and certpool with error: x509: certificate signed by unknown authority",
		},
		"Failure - cert not verifiable from root cert": {
			...
			expectedErr:   "cannot verify the cert with the provided root chain and certpool with error: x509: certificate is not authorized to sign other certificates",
		},
