...
func TestIntoResourceFile(t *testing.T) {
	cases := []struct {
		...
		{
			// Verifies that environment variables passed under global.proxy.env are applied properly
			in:                           "hello.yaml",
			want:                         "hello-proxy-env-var.injected",
			includeIPRanges:              DefaultIncludeIPRanges,
			includeInboundPorts:          DefaultIncludeInboundPorts,
			statusPort:                   DefaultStatusPort,
			readinessInitialDelaySeconds: DefaultReadinessInitialDelaySeconds,
		...
		{
			// Verifies that environment variables passed via "sidecar.istio.io/environmentOverride" annotation are applied properly
			in:                           "hello-proxy-env-var-annotation.yaml",
			want:                         "hello-proxy-env-var-annotation.injected",
			includeIPRanges:              DefaultIncludeIPRanges,
			includeInboundPorts:          DefaultIncludeInboundPorts,
			statusPort:                   DefaultStatusPort,
			readinessInitialDelaySeconds: DefaultReadinessInitialDelaySeconds,
	...
	for i, c := range cases {
		...
		t.Run(testName, func(t *testing.T) {
			...
			params := &Params{
				...
				ProxyEnvironmentVariables:    c.proxyEnvironmentVariables,
			}
