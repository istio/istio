...
function update_version() {
    ...
    sed -i "s|hub: registry.istio.io/release|hub: ${DOCKER_HUB}|g" "${RELEASE_DIR}"/profiles/*.yaml
    sed -i "s|tag: .*-latest-daily|tag: ${DOCKER_TAG}|g"      "${RELEASE_DIR}"/profiles/*.yaml
    # Update version string in global.yaml.
    sed -i "s|hub: registry.istio.io/release|hub: ${DOCKER_HUB}|g" "${RELEASE_DIR}"/charts/global.yaml
