...
/*
func TestAddListenerSockFileDelError(t *testing.T) {
	dir, err := ioutil.TempDir("", "test-binder")
	if err != nil {
		panic(err)
	}
	...
	gotErr := b.addListener("test")
	if gotErr == nil {
		t.Error("Expected to get a file remove error. Got none")
	}
	fmt.Println("gotErr %v", gotErr)
	expectedErr := fmt.Sprintf("File %s exists and unable to remove.", socketFile)
	if gotErr.Error() != expectedErr {
		t.Errorf("Expected %s error got %s", expectedErr, gotErr.Error())
	}
	// Allow removal now!
	os.Chmod(socketFile, 0666)
}

func TestAddListenerListenerError(t *testing.T) {
	dir, err := ioutil.TempDir("", "test-binder")
	if err != nil {
		panic(err)
	}
	...
	writeDefaultCredentials(dir, UID)

	// Mock net.Listen
	origListener := net.Listen
	net.Listen = func(net, laddr string) (net.Listener, error) {
		return nil, errors.New("mock error")
	}
	defer func() {
		net.Listen = origListener
	}()
	...
	gotErr := b.addListener("test")
	if gotErr == nil {
		t.Error("Expected to get a listener error. Got none")
	}
	expectedErr := fmt.Sprintf("failed to listen at %s error %s", filepath.Join(mountDir, SocketFilename), "mock error")
	if gotErr.Error() != expectedErr {
		t.Errorf("Expected %s error got %s", expectedErr, gotErr.Error())
	}
}
