...
func TestProxyQueue(t *testing.T) {
	...
	for p := 0; p < 100; p++ {
		proxies = append(proxies, &Connection{identifier: fmt.Sprintf("proxy-%d", p)})
	}
	...
	t.Run("concurrent", func(t *testing.T) {
		...
		key := func(p *Connection, eds string) string { return fmt.Sprintf("%s~%s", p.identifier, eds) }
	...
	t.Run("concurrent with deterministic order", func(t *testing.T) {
		...
		p := NewPushQueue()
		defer p.ShutDown()
		con := &Connection{identifier: "proxy-test"}
