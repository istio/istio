...
var knownTypes = map[string]schemaType{
	...
	model.AuthenticationClusterPolicy.Type: {
		schema: model.AuthenticationClusterPolicy,
		object: &ClusterPolicy{
			TypeMeta: meta_v1.TypeMeta{
				Kind:       "ClusterPolicy",
				APIVersion: apiVersion(&model.AuthenticationClusterPolicy),
			},
		},
		collection: &ClusterPolicyList{},
	},
...
// ClusterPolicy is the generic Kubernetes API object wrapper
...
// GetSpec from a wrapper
func (in *ClusterPolicy) GetSpec() map[string]interface{} {
	return in.Spec
}

// SetSpec for a wrapper
func (in *ClusterPolicy) SetSpec(spec map[string]interface{}) {
	in.Spec = spec
}

// GetObjectMeta from a wrapper
func (in *ClusterPolicy) GetObjectMeta() meta_v1.ObjectMeta {
	return in.ObjectMeta
}

// SetObjectMeta for a wrapper
func (in *ClusterPolicy) SetObjectMeta(metadata meta_v1.ObjectMeta) {
	in.ObjectMeta = metadata
}

// ClusterPolicyList is the generic Kubernetes API list wrapper
...
// GetItems from a wrapper
func (in *ClusterPolicyList) GetItems() []IstioObject {
	out := make([]IstioObject, len(in.Items))
	for i := range in.Items {
		out[i] = &in.Items[i]
	}
