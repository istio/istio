...
type ServiceEntryStore struct { // nolint:golint
	...
	foreignRegistryInstancesByIP map[string]*model.ForeignInstance
	...
	workloadHandlers          []func(*model.ForeignInstance, model.Event)
}
...
func (s *ServiceEntryStore) workloadEntryHandler(old, curr model.Config, event model.Event) {
	...
	if len(s.workloadHandlers) > 0 {
		si := convertWorkloadEntryToForeignInstances(curr.Namespace, wle)
		if si != nil {
...
func (s *ServiceEntryStore) ForeignServiceInstanceHandler(si *model.ForeignInstance, event model.Event) {
	key := configKey{
	...
	for _, se := range entries {
		...
		instance := convertForeignServiceInstances(si.Endpoint, se.services, se.entry)
		instances = append(instances, instance...)
	}
...
// AppendInstanceHandler adds instance event handler. Service Entries does not use these handlers.
func (s *ServiceEntryStore) AppendWorkloadHandler(h func(*model.ForeignInstance, model.Event)) error {
...
func (s *ServiceEntryStore) maybeRefreshIndexes() {
	...
	for ip, foreignInstance := range s.foreignRegistryInstancesByIP {
		key := configKey{
			kind:      foreignInstanceConfigType,
			name:      ip,
			namespace: foreignInstance.Namespace,
		}

		instances := []*model.ServiceInstance{}
		// We will only select entries in the same namespace
		entries := seWithSelectorByNamespace[foreignInstance.Namespace]
		for _, se := range entries {
			...
			instance := convertForeignServiceInstances(foreignInstance.Endpoint, se.services, se.entry)
			instances = append(instances, instance...)
		}
