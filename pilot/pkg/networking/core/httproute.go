...
func generateVirtualHostDomains(service *model.Service, listenerPort int, port int, node *model.Proxy) ([]string, []string) {
	...
	// For DYNAMIC_DNS services, only use the exact hostname to avoid wildcard domain conflicts.
	// The DFP HTTP filter will handle dynamic resolution based on the Host header.
	if service.Resolution == model.DynamicDNS {
		return []string{service.Hostname.String()}, []string{}
	}
