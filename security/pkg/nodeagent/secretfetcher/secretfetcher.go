...
const (
	...
	// spiffeTrustBundlePrefix is the prefix in ConfigMaps for SPIFFE trust bundle.
	spiffeTrustBundlePrefix = "spiffe-tb-"

	// The key for the trust domain.
	trustDomainKey = "trust_domain"

	// The key for the trust bundle.
	trustBundleKey = "trust_bundle"

	configMapType kubeClientType = iota
	secretType
)
...
func NewSecretFetcher(ingressGatewayAgent bool, endpoint, caProviderName string, tlsFlag bool,
	...
	if ingressGatewayAgent {
		...
		ret.InitWithKubeClient(secretType, cs.CoreV1())
	} else {
		secretFetcherLog.Warnf("CA client + ConfigMap watcher.")
		...
		ret.InitWithKubeClient(configMapType, cs.CoreV1())
	}
...
var namespaceVar = env.RegisterStringVar(ingressSecretNamespace, "istio-system", "")

// InitWithKubeClient initializes SecretFetcher to watch kubernetes secrets.
func (sf *SecretFetcher) InitWithKubeClient(clientType kubeClientType, core corev1.CoreV1Interface) { // nolint:interfacer
	switch clientType {
	case secretType:
		sf.InitWithKubeSecretClientAndNs(core, namespaceVar.Get())
	case configMapType:
		sf.InitWithKubeConfigMapClientAndNs(core, namespaceVar.Get())
		secretFetcherLog.Warnf("ConfigMap watcher initialized for: %s", namespaceVar.Get())
	}
...
func (sf *SecretFetcher) InitWithKubeClientAndNs(core corev1.CoreV1Interface, namespace string) { // nolint:interfacer
	sf.InitWithKubeSecretClientAndNs(core, namespace)
}

// InitWithKubeSecretClientAndNs initializes SecretFetcher to watch kubernetes secrets.
func (sf *SecretFetcher) InitWithKubeSecretClientAndNs(core corev1.CoreV1Interface, namespace string) { // nolint:interfacer
...
// InitWithKubeConfigMapClientAndNs initializes SecretFetcher to watch kubernetes secrets.
func (sf *SecretFetcher) InitWithKubeConfigMapClientAndNs(core corev1.CoreV1Interface, namespace string) { // nolint:interfacer
	istioConfigMapSelector := fields.SelectorFromSet(nil).String()
	scrtLW := &cache.ListWatch{
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			options.FieldSelector = istioConfigMapSelector
	...
	sf.scrtStore, sf.scrtController =
		cache.NewInformer(scrtLW, &v1.ConfigMap{}, resyncPeriod, cache.ResourceEventHandlerFuncs{
			AddFunc:    sf.configMapAdded,
			DeleteFunc: sf.configMapDeleted,
			UpdateFunc: sf.configMapUpdated,
		})
...
// extractSpiffeTBIntoSecretItem extracts a serials of client CA
// certificate from the k8s Secret into a pair of SecretItems. Returns SecretItems.
func extractSpiffeTBIntoSecretItem(cm *v1.ConfigMap, t time.Time) *model.SecretItem {
	...
	secretFetcherLog.Warnf(
		"extractSpiffeTBIntoSecretItem:\n  trustDomain: %s\n  trustBundle: %s", newSecret.ResourceName, string(newSecret.RootCert))

	return newSecret
}

func (sf *SecretFetcher) configMapAdded(obj interface{}) {
	cm, ok := obj.(*v1.ConfigMap)
	if !ok {
		secretFetcherLog.Warnf("Failed to convert to ConfigMap object: %v", obj)
		return
	}

	resourceName := cm.GetName()
	secretFetcherLog.Warnf("ConfigMapAdded: %s", resourceName)
	if !strings.HasPrefix(resourceName, spiffeTrustBundlePrefix) {
		secretFetcherLog.Debugf("ConfigMap %s is not a trust bundle, skip adding it", resourceName)
		return
	} else {
		secretFetcherLog.Warnf("ConfigMap %s is SPIFFE trust bundle", resourceName)
	}

	t := time.Now()
	caNewSecret := extractSpiffeTBIntoSecretItem(cm, t)

	// Load CA cert from CA only k8s secret and update cache.
	sf.secrets.Delete(caNewSecret.ResourceName)
	sf.secrets.Store(caNewSecret.ResourceName, *caNewSecret)
	secretFetcherLog.Debugf("secret %s is added as a root cert", caNewSecret.ResourceName)
	if sf.AddCache != nil {
		sf.AddCache(caNewSecret.ResourceName, *caNewSecret)
	}
}

func (sf *SecretFetcher) configMapDeleted(obj interface{}) {
	// TODO
	secretFetcherLog.Debugf("Unimplemented configMapDeleted")
}

func (sf *SecretFetcher) configMapUpdated(oldObj, newObj interface{}) {
	// TODO
	secretFetcherLog.Debugf("Unimplemented configMapUpdated")
}
