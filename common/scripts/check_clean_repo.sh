...
function write_patch_file() {
    ...
    echo "You can also try applying the patch file from the build artifacts: 'git apply <(curl -sL ${PROW_ARTIFACTS_BASE:-https://gcsweb.istio.io/gcs/istio-prow}/pr-logs/pull/${REPO_OWNER}_${REPO_NAME}/${PULL_NUMBER}/${JOB_NAME}/${BUILD_ID}/${PATCH_OUT}"
}
