...
func removeOldChains(cfg *config.Config, ext dep.Dependencies, cmd string) {
	...
	if redirectDNS {
		common.HandleDNSUDP(common.DeleteOps, builder.NewIptablesBuilder(nil), ext, cmd, cfg.ProxyUID, cfg.ProxyGID, cfg.DNSServersV4, cfg.DNSServersV6, cfg.CaptureAllDNS)
	}
