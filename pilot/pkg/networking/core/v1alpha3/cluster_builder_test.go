...
func TestApplyDestinationRule(t *testing.T) {
	...
	cases := []struct {
		...
		expectedSubsetClusters []*clusterv3.Cluster
	}{
		...
		},
		{
			...
			cluster:     &clusterv3.Cluster{Name: "foo", ClusterDiscoveryType: &clusterv3.Cluster_Type{Type: clusterv3.Cluster_EDS}},
			clusterMode: DefaultClusterMode,
			service:     service,
			port:        servicePort[0],
			proxy:       &model.Proxy{},
...
func compareClusters(t *testing.T, ec *clusterv3.Cluster, gc *clusterv3.Cluster) {
	...
	if ec.GetType() == clusterv3.Cluster_EDS && ec.EdsClusterConfig.ServiceName != gc.EdsClusterConfig.ServiceName {
...
func TestApplyEdsConfig(t *testing.T) {
	...
	cases := []struct {
		name      string
		cluster   *clusterv3.Cluster
		edsConfig *clusterv3.Cluster_EdsClusterConfig
	}{
		{
			name:      "non eds type of cluster",
			cluster:   &clusterv3.Cluster{Name: "foo", ClusterDiscoveryType: &clusterv3.Cluster_Type{Type: clusterv3.Cluster_STRICT_DNS}},
			edsConfig: nil,
		},
		{
...
func TestBuildDefaultCluster(t *testing.T) {
	...
	cases := []struct {
		name            string
		clusterName     string
		discovery       clusterv3.Cluster_DiscoveryType
		endpoints       []*endpointv3.LocalityLbEndpoints
		direction       model.TrafficDirection
		external        bool
		expectedCluster *clusterv3.Cluster
	}{
		{
			name:        "default EDS cluster",
			clusterName: "foo",
			discovery:   clusterv3.Cluster_EDS,
			endpoints:   nil,
			...
			expectedCluster: &clusterv3.Cluster{
				...
				CircuitBreakers: &clusterv3.CircuitBreakers{
					Thresholds: []*clusterv3.CircuitBreakers_Thresholds{&defaultCircuitBreakerThresholds},
				},
			},
		},
		{
			...
			clusterName:     "foo",
			discovery:       clusterv3.Cluster_STATIC,
			endpoints:       nil,
			direction:       model.TrafficDirectionOutbound,
			external:        false,
			expectedCluster: nil,
		},
		{
			...
			clusterName:     "foo",
			discovery:       clusterv3.Cluster_STRICT_DNS,
			endpoints:       nil,
			direction:       model.TrafficDirectionOutbound,
			external:        false,
			expectedCluster: nil,
...
func TestBuildPassthroughClusters(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if clusters[0].GetType() != clusterv3.Cluster_ORIGINAL_DST || clusters[0].GetLbPolicy() != clusterv3.Cluster_CLUSTER_PROVIDED {
