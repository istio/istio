...
func TestSecretController(t *testing.T) {
	...
	for k, tc := range testCases {
		...
		controller, err := NewSecretController(createFakeCA(), defaultTTL, "cluster.local",
...
func TestSecretContent(t *testing.T) {
	...
	client := fake.NewSimpleClientset()
	controller, err := NewSecretController(createFakeCA(), defaultTTL, "cluster.local",
		defaultGracePeriodRatio, defaultMinGracePeriod, client.CoreV1(), false,
...
func TestUpdateSecret(t *testing.T) {
	...
	for k, tc := range testCases {
		client := fake.NewSimpleClientset()
		controller, err := NewSecretController(createFakeCA(), time.Hour, "cluster.local",
			tc.gracePeriodRatio, tc.minGracePeriod, client.CoreV1(), false, metav1.NamespaceAll, nil)
		if err != nil {
			t.Errorf("failed to create secret controller: %v", err)
		}
