...
const (
	mixer                 = "mixer"
	outbounddisablepolicy = "*.example.orgoutbounddisablePolicy"
	outboundenablepolicy  = "*.example.orgoutboundenablePolicy"
)
...
func TestBuildSharedPushState(t *testing.T) {
	...
	configgen.BuildSharedPushState(env, env.PushContext)
	filterConfig := configgen.PerRouteFilterConfig[mixer]
	c, ok := filterConfig.(map[string]*types.Struct)
	if !ok {
	...
	if !ok {
		t.Fatalf("can not get expected Filter config: " + outbounddisablepolicy)
	}

	_, ok = c[outboundenablepolicy]
	if !ok {
		t.Fatalf("can not get expected Filter config: " + outboundenablepolicy)
	}
}
