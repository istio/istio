...
func TestBuildHTTPRoutes(t *testing.T) {
	...
	t.Run("for virtual service", func(t *testing.T) {
		...
		g.Expect(routes[0].GetRoute().MaxStreamDuration.GetMaxStreamDuration().GetSeconds()).To(gomega.Equal(int64(0)))
		g.Expect(routes[0].GetRoute().MaxStreamDuration.GetGrpcTimeoutHeaderMax().GetSeconds()).To(gomega.Equal(int64(0)))
	})
	...
	t.Run("for virtual service with timeout", func(t *testing.T) {
		...
		g.Expect(routes[0].GetRoute().MaxStreamDuration.GetMaxStreamDuration().GetSeconds()).To(gomega.Equal(int64(10)))
	})
