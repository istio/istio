...
type CertOpts struct {
	...
	// Cert Signer info. From the gRPC 'CertSigner' metadata in case of Istio CA, only used with CERT_SIGNER_DOMAIN
	CertSigner string
}
...
func NewSelfSignedIstioCAOptions(ctx context.Context,
	...
	err = b.RetryWithContext(ctx, func() error {
		...
		if err == nil {
			...
			// This uses the old-style Istio names (not tls.crt), for compatibility with older revisions and consistency.
...
// This is a self-signed certificate generated by Istiod (or user), that Istiod can rotate and is managing.
func loadCASecrets(client corev1.CoreV1Interface, namespace string, caCertName string, rootCertFile string, caOpts *IstioCAOptions) error {
	...
	if err == nil {
		...
		pkiCaLog.Infof("Load signing key and cert from existing self-signed CA secret %s/%s", caSecret.Namespace, caSecret.Name)
		...
		// TODO: find out why this was added. Should not be an issue as normal behavior will trigger a different code path
		// if the cacerts are mounted, and for self-signed the file will not exist.
		...
		if caOpts.KeyCertBundle, err = util.NewVerifiedKeyCertBundleFromPem(caCertData,
			caKeyData, nil, rootCerts); err != nil {
			return fmt.Errorf("failed to create CA KeyCertBundle (%v)", err)
		}
...
// loadCacertSecret handles the load of a chained CA - since the Istio CA certificate is not self-signed
// Istiod can't rotate it, and we have to handle certificate chains. This should work the same as the 'file mounted'
// option - but handles the case where Istiod is remote and can't mount the cacert Secret as a file ( or get it
// mounted trough CSI volume or helpers)
func loadCacertSecret(client corev1.CoreV1Interface, namespace string, caCertName string, caOpts *IstioCAOptions) error {
	...
	if rootCertData != nil {
		// We didn't find the new-style tls.crt - fallback to the old file names and style.
		rootCertData = caSecret.Data[RootCertFile]
		privData = caSecret.Data[CAPrivateKeyFile]

		leafData := caSecret.Data[CACertFile]
		//
		intermediariesData := caSecret.Data[CertChainFile]
		crtData = leafData
		if intermediariesData != nil {
			// Concatenate the leaf and intermediaries - to be consistent with the new tls.crt style.
			crtData = append(crtData, '\n')
			crtData = append(crtData, intermediariesData...)
		} else {
			pkiCaLog.Warnf("Load legacy cacerts %s/%s with missing cert-chain.pem", caSecret.Namespace, caSecret.Name)
		}
...
// Roots are not returned - unlike the K8S RA with CERT_SIGNER_DOMAIN which sometimes returns the roots, sometimes
// it doesn't.
