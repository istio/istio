...
func GetProxyInfo(kubeClient kube.CLIClient, istioNamespace string, port int) (*[]istioVersion.ProxyInfo, error) {
	// Ask Pilot for the Envoy sidecar sync status, which includes the sidecar version info
	allSyncz, err := kubeClient.AllDiscoveryDo(context.TODO(), istioNamespace, "debug/syncz", port)
	if err != nil {
		return nil, err
	}
...
func GetIDsFromProxyInfo(kubeClient kube.CLIClient, istioNamespace string, port int) ([]string, error) {
	var IDs []string
	pi, err := GetProxyInfo(kubeClient, istioNamespace, port)
	if err != nil {
