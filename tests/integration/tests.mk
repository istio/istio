...
test.integration.%.analyze: | $(JUNIT_REPORT)
	$(GO) test -p 1 ${T} -vet=off ./tests/integration/$(subst .,/,$*)/... -timeout 30m \
...
test.integration.%.kube.presubmit: | $(JUNIT_REPORT)
	PATH=${PATH}:${ISTIO_OUT} $(GO) test -p 1 ${T} -vet=off ./tests/integration/$(subst .,/,$*)/... -timeout 30m \
...
test.integration.kube.presubmit: | $(JUNIT_REPORT)
	PATH=${PATH}:${ISTIO_OUT} $(GO) test -p 1 ${T} -vet=off  $(shell go list ./tests/integration/... | grep -v /qualification | grep -v /examples) -timeout 30m \
	${_INTEGRATION_TEST_FLAGS} ${_INTEGRATION_TEST_SELECT_FLAGS} \
	2>&1 | tee >($(JUNIT_REPORT) > $(JUNIT_OUT))
...
test.integration.kube.reachability: | $(JUNIT_REPORT)
	PATH=${PATH}:${ISTIO_OUT} $(GO) test -p 1 ${T} -vet=off ./tests/integration/security/ -timeout 30m \
	${_INTEGRATION_TEST_FLAGS} \
