...
func TestTrustDomainValidation(t *testing.T) {
	framework.NewTest(t).Features("security.customca.trust-domain-validation").Run(
		func(ctx framework.TestContext) {
			...
			trustDomains := map[string]struct {
				...
				// TODO: Re-enable tests in trust-domain foo after custom CA integration that signs certs using a known root-cert
				/*
					"foo": {
