...
func TestDialout_Basic(t *testing.T) {
	framework.NewTest(t).
		Run(func(ctx framework.TestContext) {
			srv := mcpserver.GetSinkHandleOrFail(t, ctx)

			var g galley.Instance
			if ctx.Environment().EnvironmentName() == environment.Kube {
				// Since this runs after Istio is setup on Kubernetes (which also includes
				// deploying Galley) we cannot pass configuration here. All we can do is to
				// get a handle to the Galley pods.
				g = galley.GetGalleyHandleInKubernetesOrFail(t, ctx)
			} else {
				g = galley.NewNativeGalleyOrFail(t, ctx, galley.Config{SinkAddress: srv.Address()})
			}
