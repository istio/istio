...
func TestResolveHostname(t *testing.T) {
	cases := []struct {
		...
		svc  *routerule.IstioService
		want string
	}{
		{
			meta: model.ConfigMeta{Namespace: "default", Domain: "cluster.local"},
			svc:  &routerule.IstioService{Name: "hello"},
		...
		{
			meta: model.ConfigMeta{Namespace: "default", Domain: "cluster.local"},
			svc:  &routerule.IstioService{Service: "reviews.service.consul"},
			want: "reviews.service.consul",
		},
		{
			meta: model.ConfigMeta{Namespace: "foo", Domain: "foo"},
			svc: &routerule.IstioService{Name: "hello", Service: "reviews.service.consul",
				Namespace: "default", Domain: "svc.cluster.local"},
			want: "reviews.service.consul",
		},
		{
			meta: model.ConfigMeta{Namespace: "default", Domain: "cluster.local"},
			svc:  &routerule.IstioService{Service: "*cnn.com"},
			want: "*cnn.com",
		},
		{
			meta: model.ConfigMeta{Namespace: "foo", Domain: "foo"},
			svc: &routerule.IstioService{Name: "hello", Service: "*cnn.com",
				Namespace: "default", Domain: "svc.cluster.local"},
			want: "*cnn.com",
		},
	}
...
func TestMatchSource(t *testing.T) {
	cases := []struct {
		...
		{
			meta:      model.ConfigMeta{Name: "test", Namespace: "default", Domain: "cluster.local"},
			svc:       &routerule.IstioService{Name: "world"},
		...
		{
			meta:      model.ConfigMeta{Name: "test", Namespace: "default", Domain: "cluster.local"},
			svc:       &routerule.IstioService{Name: "hello", Labels: map[string]string{"version": "v0"}},
			instances: []*model.ServiceInstance{mock.MakeInstance(mock.HelloService, mock.PortHTTP, 0)},
			want:      true,
		},
	}
	...
	rules := []model.Config{
		{
			...
			Spec:       &routerule.RouteRule{Precedence: 2},
		},
		{
			ConfigMeta: model.ConfigMeta{Name: "b"},
			Spec:       &routerule.RouteRule{Precedence: 3},
		},
		{
...
func TestRouteRules(t *testing.T) {
	...
	routerule1 := &routerule.RouteRule{
...
func TestEgressRules(t *testing.T) {
	...
	want := map[string]*routerule.EgressRule{
...
func TestPolicy(t *testing.T) {
	...
	policy1 := &routerule.DestinationPolicy{
		Source: &routerule.IstioService{
			Name:   "hello",
