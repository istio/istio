...
// all of this function can be executed under a read lock
func (s *Controller) isRedundantEvent(wi *model.WorkloadInstance, event model.Event) bool {
...
func (s *Controller) WorkloadInstanceHandler(wi *model.WorkloadInstance, event model.Event) {
	...
	if s.isRedundantEvent(wi, event) {
		return
	}
