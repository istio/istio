...
func TestCopyAndProcessMetrics(t *testing.T) {
	type test struct {
		name    string
		buf     []byte
		metrics string
	...
	testCases := []test{
		...
		},
		{
			...
			buf:  make([]byte, 8),
			metrics: `# TYPE my_metric counter
my_metric{} 0

# TYPE my_other_metric counter
my_other_metric{} 0
`,
			want: `# TYPE my_metric counter
my_metric{} 0
# TYPE my_other_metric counter
my_other_metric{} 0
`,
	...
	testFn := func(t *testing.T, tc test) {
		reader := strings.NewReader(tc.metrics)
		writer := &bytes.Buffer{}
		written, err := copyAndProcessMetrics(writer, reader, tc.buf)
		if err != nil {
