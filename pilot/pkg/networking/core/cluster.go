...
func (configgen *ConfigGeneratorImpl) buildClusters(proxy *model.Proxy, req *model.PushRequest,
	...
	case model.Waypoint:
		...
		outboundPatcher := clusterPatcher{efw: envoyFilterPatches, pctx: networking.EnvoyFilter_WAYPOINT}
		...
		resources = append(resources, ob...)
		// Setup inbound clusters
		wpClusters := configgen.buildWaypointInboundClusters(cb, proxy, req.Push, wps.services)
		inboundPatcher := clusterPatcher{efw: envoyFilterPatches, pctx: networking.EnvoyFilter_WAYPOINT}
		clusters = inboundPatcher.conditionallyAppend(clusters, nil, wpClusters...)
		clusters = append(clusters, inboundPatcher.insertedClusters()...)
	default: // Gateways
