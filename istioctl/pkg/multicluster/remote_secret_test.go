		...
		t.Run(fmt.Sprintf("[%v] %v", i, c.testName), func(tt *testing.T) {
			...
			if c.wantErrStr != "" {
				...
				case SecretTypeConfig:
					...
					key = configSecretKey
				default:
					secretName = constants.RemoteSecretPrefix + string(kubeSystemNamespaceUID)
...
func TestCreateRemoteKubeconfig(t *testing.T) {
	...
	fakeRemoteSecretName := remoteSecretNameFromClusterName(fakeClusterName)
