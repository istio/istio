...
func (m *IstioOperator) DeepCopyInto(out proto.Message) {
	bytes, err := util.MarshalWithJSONPB(m)
	if err != nil {
		log.Errorf("Error in IstioOperator DeepCopyInto Marshal: %s", err.Error())
		return
	}
	if err = util.UnmarshalWithJSONPB(bytes, out, false); err != nil {
		log.Errorf("Error in IstioOperator DeepCopyInto Unmarshal: %s", err.Error())
	}
}
