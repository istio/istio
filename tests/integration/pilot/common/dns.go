...
// RunDNSAutoAllocationTests verifies that DNS capture and auto IP allocation via
// ISTIO_META_DNS_CAPTURE + ISTIO_META_DNS_AUTO_ALLOCATE correctly route traffic
// to a ServiceEntry host with no real DNS entry.
func RunDNSAutoAllocationTests(t framework.TestContext, apps deployment.SingleNamespaceView) {
	t.Helper()
...
`
	...
	instances := echodeployment.New(t, t.AllClusters().Configs()...).WithConfig(echo.Config{Namespace: ns, Service: "a"}).BuildOrFail(t)
