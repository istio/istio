...
func (m *IptablesTrafficManager) CreateInpodRules(log *istiolog.Scope, podOverrides config.PodLevelOverrides, _ string) error {
...
func (m *IptablesTrafficManager) DeleteInpodRules(log *istiolog.Scope, _ string) error {
	if m.podIptables == nil {
		return fmt.Errorf("pod iptables configurator not available (this is likely a host-only traffic manager)")
	}
