...
func (c *Counter) Log(msg string) {
	Info("%s : count %d avg %.8g +/- %.4g min %g max %g sum %.9g",
		msg, c.Count, c.Avg(), c.StdDev(), c.Min, c.Max, c.Sum)
}
...
func init() {
	...
	if idx != numBuckets-1 {
		Fatal("Bug in creating histogram buckets idx %d vs numbuckets %d (last val %d)", idx, numBuckets, lastV)
	}
...
func (h *Histogram) Transfer(src *Histogram) {
	...
	if src.Offset != h.Offset {
		Fatal("Incompatible offsets in Histogram Transfer %f %f", src.Offset, h.Offset)
	}
	if src.Divider != h.Divider {
		Fatal("Incompatible scale in Histogram Transfer %f %f", src.Divider, h.Divider)
	}
	if src.Count == 0 {
		return
	}
