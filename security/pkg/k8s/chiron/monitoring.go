...
var (
	serviceAccountCreationCounts = prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace: "chiron",
		Subsystem: "webhook_controller",
		Name:      "svc_acc_created_cert_count",
		Help:      "The number of certificates created due to webhook service accounts creation.",
	}, []string{})
	...
	serviceAccountDeletionCounts = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help:      "The number of certificates deleted due to webhook service accounts deletion.",
	}, []string{})
	...
	secretDeletionCounts = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help:      "The number of certificates recreated due to webhook secret deletion (service account still exists).",
	}, []string{})
