...
func createPerRevisionElection(t *testing.T,
	...
	t.Helper()
	return createElectionMulticluster(t, name, revision, false, true, watcher, true, expectLeader, client)
}
...
func TestPrioritizedMulticlusterLeaderElection(t *testing.T) {
	...
	_, stop := createElectionMulticluster(t, "pod1", "green", true, false, watcher, true, true, client)
	// Second pod, revision "red", steals the leader lock from "green" since it is the default revision
	_, stop2 := createElectionMulticluster(t, "pod2", "red", true, false, watcher, true, true, client)
	// Third pod with revision "red" comes in and can take the lock since it is a local revision "red"
	_, stop3 := createElectionMulticluster(t, "pod3", "red", false, false, watcher, true, true, client)
	// Fourth pod with revision "red" cannot take the lock since it is remote
	_, stop4 := createElectionMulticluster(t, "pod4", "red", true, false, watcher, true, false, client)
	close(stop4)
	close(stop3)
	close(stop2)
	close(stop)
