...
func Combine(name string, analyzers ...analysis.Analyzer) *CombinedAnalyzer {
...
func (c *CombinedAnalyzer) Metadata() analysis.Metadata {
...
func (c *CombinedAnalyzer) Analyze(ctx analysis.Context) {
	...
	for _, msg := range ctx.Messages() {
		// these resources are invalid, and could cause other analyzers to panic
		ctx.Skip(msg.Resource)
	}
	for _, a := range c.analyzers {
		if _, ok := a.(*schema.ValidationAnalyzer); ok {
...
func runAnalyzer(ctx analysis.Context, a analysis.Analyzer, name string) bool {
...
func combineInputs(analyzers []analysis.Analyzer) collection.Names {
