...
func (s *ServiceEntryStore) WorkloadInstanceHandler(wi *model.WorkloadInstance, event model.Event) {
	...
	// we only update the service registry in the below conditions:
	// 1. pods are becoming ready => InTransitState == true
	// 2. pods are becoming unready => InTransitState == true
	// 3. pods are being deleted
	if !wi.InTransitState && event != model.EventDelete {
		return
	}
	key := configKey{
