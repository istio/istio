...
type Options struct {
	...
	// WorkloadCache contains the WorkloadInstances representing WorkloadEntry
...
func NewController(kubeClient kubelib.Client, options Options) *Controller {
	...
	c := &Controller{
		...
		workloadCache:               options.WorkloadCache,
...
func (c *Controller) serviceInstancesFromWorkloadInstances(svc *model.Service, reqSvcPort int) []*model.ServiceInstance {
	...
	workloadInstances := c.workloadCache.All()
...
func (c *Controller) collectWorkloadInstanceEndpoints(svc *model.Service) []*model.IstioEndpoint {
	...
	if svc.Resolution != model.ClientSideLB || len(svc.Ports) == 0 {
		return nil
	}
...
func (c *Controller) GetProxyServiceInstances(proxy *model.Proxy) []*model.ServiceInstance {
	if len(proxy.IPAddresses) > 0 {
		...
		if workload := c.workloadCache.ByIPNetwork(proxyIP, proxy.Metadata.Network); workload != nil {
