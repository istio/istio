...
func TestBuildDeltaClusters(t *testing.T) {
	...
	sidecarWithEgressHostsVs := &networking.Sidecar{
		Egress: []*networking.IstioEgressListener{
			{
				Hosts: []string{"bar/testvs.com"},
			},
		},
	}

	virtualService := &networking.VirtualService{
		Hosts: []string{"testvs.com"},
		Http: []*networking.HTTPRoute{
			{
				Route: []*networking.HTTPRouteDestination{
					{
						Destination: &networking.Destination{Host: "test.com"},
					},
				},
			},
		},
	...
	virtualServiceUpdated := &networking.VirtualService{
		Hosts: []string{"testvs.com"},
		Http: []*networking.HTTPRoute{
			{
				Route: []*networking.HTTPRouteDestination{
					{
						Destination: &networking.Destination{Host: "testnew.com"},
					},
				},
			},
		},
	...
	testCases := []struct {
		...
		{
			name: "sidecar with vs config update changing service selection",
			prevConfigs: []config.Config{{
				Meta: config.Meta{
					GroupVersionKind: gvk.Sidecar,
					Name:             "default",
			...
			configs: []config.Config{{
				...
				Spec: virtualServiceUpdated,
			}},
			services:             []*model.Service{testService1, testService2},
			configUpdated:        sets.New(model.ConfigKey{Kind: kind.VirtualService, Name: "testvs.com", Namespace: TestServiceNamespace}),
			watchedResourceNames: []string{"outbound|8080||test.com"},
			usedDelta:            true,
			removedClusters:      []string{"outbound|8080||test.com"},
			expectedClusters: []string{
