...
func TestBadWasmRemoteLoad(t *testing.T) {
	framework.NewTest(t).
		...
		Run(func(ctx framework.TestContext) {
			badWasmTestHelper(t, ctx, "testdata/bad-filter.yaml", false, true)
		})
}
...
func TestBadWasmWithFailOpen(t *testing.T) {
	framework.NewTest(t).
		...
		Run(func(ctx framework.TestContext) {
			...
			badWasmTestHelper(t, ctx, "testdata/bad-wasm-envoy-filter-fail-open.yaml", true, false)
		})
}

func badWasmTestHelper(t *testing.T, ctx framework.TestContext, filterConfigPath string, restartTarget bool, ecdsShouldReject bool) {
	ctx.Helper()
	// Test bad wasm remote load in only one cluster.
	// There is no need to repeat the same testing logic in multiple clusters.
	to := common.GetClientInstances().GetOrFail(ctx, echo.InCluster(ctx.Clusters().Default()))
	...
	content, err := os.ReadFile(filterConfigPath)
	if err != nil {
		t.Fatal(err)
	}
	ctx.ConfigIstio().ApplyYAML(common.GetAppNamespace().Name(), string(content))
	if restartTarget {
		target := common.GetServerInstances().GetOrFail(t, echo.InCluster(ctx.Clusters().Default()))
	...
	if ecdsShouldReject && ctx.Clusters().Default().IsPrimary() { // Only check istiod if running locally (i.e., not an external control plane)
