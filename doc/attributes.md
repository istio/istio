...
In a standard Istio deployment (using the Istio proxy), the system will produce the following set of attributes. This is considered the standard, or canonical, attribute set for Istio.

| Name| Type| Description| Kubernetes Example| HTTP Header Name| Notes|
|-----|-----|------------|-------------------|-----------------|------|
...
| source.name| string| The fully qualified name of the application that sent the request.| redis-master.namespace.deps.cluster.local| x-envoy-downstream-service-cluster| Using a name this is not a resolvable DNS name. MIght need to tweak to avoid collision|
...
| source.user| string| The user running the source application.| service-account@namespace.cluster.local| | |
...
| target.service| dns_name| The hostname that was the target of the request.| my-svc.my-namespace.svc.cluster.local| :authority / Host| Server-side proxy could also see Pod hostnames being used. Need to dig in|
...
| origin.ip| ip_address| The IP address of the originating application| | x-envoy-external-address, x-forwarded-for| |
