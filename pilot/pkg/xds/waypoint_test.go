...
spec:
  ...
  resolution: DYNAMIC_DNS`
	...
	g.Expect(cluster.TransportSocket.GetTypedConfig().TypeUrl).To(Equal("type.googleapis.com/envoy.extensions.transport_sockets.internal_upstream.v3.InternalUpstreamTransport"))
...
spec:
  ...
  trafficPolicy:
	...
	g.Expect(tlsCluster1.TransportSocket.GetTypedConfig().TypeUrl).To(Equal("type.googleapis.com/envoy.extensions.transport_sockets.internal_upstream.v3.InternalUpstreamTransport"))
	tlsCluster2 := clusters["inbound-vip|81|http|*.domain.com"]
	g.Expect(tlsCluster2).ToNot(BeNil())
	g.Expect(tlsCluster2.TransportSocket.GetTypedConfig().TypeUrl).To(Equal("type.googleapis.com/envoy.extensions.transport_sockets.internal_upstream.v3.InternalUpstreamTransport"))
