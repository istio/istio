...
func getProfileYAML(installPackagePath, profileOrPath string) (string, error) {
	// If charts are a file path and profile is not build in and not a file path, transform it to the file path.
	if installPackagePath != "" && !helm.IsBuiltinProfileName(profileOrPath) && !util.IsFilePath(profileOrPath) {
