...
func (configgen *ConfigGeneratorImpl) MeshConfigChanged(mesh *meshconfig.MeshConfig) {
	...
	// Reset blackhole and passthrough clusters
	cb := NewClusterBuilder(nil, &model.PushContext{Mesh: mesh})
	configgen.CDSCache.SetBlackHoleCluster(cb.buildBlackHoleCluster())
	configgen.CDSCache.SetPassthroughCluster(cb.buildDefaultPassthroughCluster())
}
