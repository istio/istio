...
func TestIntoResourceFile(t *testing.T) {
	...
	for i, c := range cases {
		...
		t.Run(testName, func(t *testing.T) {
			...
			if err = IntoResourceFile(sidecarTemplate, valuesConfig, &m, true, in, &got); err != nil {
...
func TestInvalidAnnotations(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.annotation, func(t *testing.T) {
			...
			defer func() { _ = in.Close() }()
			var got bytes.Buffer
			if err = IntoResourceFile(sidecarTemplate, valuesConfig, params.Mesh, true, in, &got); err == nil {
				t.Fatalf("expected error")
			} else if !strings.Contains(strings.ToLower(err.Error()), c.annotation) {
				t.Fatalf("unexpected error: %v", err)
			}
