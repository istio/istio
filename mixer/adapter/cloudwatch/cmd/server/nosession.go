...
import (
	...
	config "istio.io/istio/mixer/adapter/cloudwatch/config"
...
// NewCloudwatchNoSessionServer creates a new no session server based on given args.
func NewCloudwatchNoSessionServer(addr uint16, poolSize int, c *Cert) (*NoSession, error) {
	saddr := fmt.Sprintf(":%d", addr)

	gp := pool.NewGoroutinePool(poolSize, false)
	inf := cloudwatch.GetInfo()
	s := &NoSession{
		builder:    inf.NewBuilder(),
		env:        handler.NewEnv(0, "cloudwatch-nosession", gp),
