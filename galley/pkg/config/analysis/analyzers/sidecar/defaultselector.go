...
func (a *DefaultSelectorAnalyzer) Analyze(c analysis.Context) {
	...
	c.ForEach(metadata.IstioNetworkingV1Alpha3Sidecars.Name, func(r *resource.Instance) bool {
	...
	for ns, sList := range nsToSidecars {
		if len(sList) > 1 {
			...
			for _, r := range sList {
				c.Report(metadata.IstioNetworkingV1Alpha3Sidecars.Name, msg.NewMultipleSidecarsWithoutWorkloadSelectors(r, sNames, string(ns)))
			}
		}
	}
}
