...
func canUseOldEdsCache(req *model.PushRequest) bool {
	for cfg := range req.ConfigsUpdated {
		// peer authentication may change endpoints' tls mode, as we donot add pa as eds key (difficult),
		// so donot use old eds cache generated by an older push request whenever PA updated.
	...
	for _, clusterName := range w.ResourceNames {
		...
		if !features.EnableUnsafeAssertions {
			...
			if useOld {
				...
				// If PA has changed but the eds cache has already been refreshed in this period.
				cachedEndpoint = eds.Server.Cache.GetWithRequest(&builder, req)
			}
