...
func TestVirtualServiceGatewayValues(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	ingress := v1beta1.Ingress{
		ObjectMeta: metaV1.ObjectMeta{
			...
			Annotations: map[string]string{
				"ingress.kubernetes.io/istio-gateway": "foo-gateway",
			},
		},
		Spec: v1beta1.IngressSpec{
			Rules: []v1beta1.IngressRule{
	...
	for n, cfg := range cfgs {
		...
		if n == "one.host.com" && vs.Gateways[0] != "foo-gateway" {
			t.Error("Unexpected gateway for vs: ", vs)
		}
		if n == "two.host.com" && vs.Gateways[0] != "istio-system/-istio-autogenerated-k8s-ingress" {
			t.Error("Unexpected gateway for vs: ", vs)
		}
	}
}
...
func TestGatewayCreationNeeded(t *testing.T) {
	cases := []struct {
		...
		{annotation: "foo-gateway-name", shouldCreateGateway: false},
		{annotation: "foo-namespace/foo-gateway-name", shouldCreateGateway: false},
	}

	for i, c := range cases {
		t.Run(fmt.Sprintf("%d %s", i, c.annotation), func(t *testing.T) {
			ing := v1beta1.Ingress{
				ObjectMeta: metaV1.ObjectMeta{
					Name:        "test-ingress",
					Namespace:   "default",
			...
			mesh := mesh.DefaultMeshConfig()
			mesh.IngressControllerMode = meshconfig.MeshConfig_DEFAULT

			if c.annotation != "" {
				ing.Annotations["ingress.kubernetes.io/istio-gateway"] = c.annotation
			}

			if c.shouldCreateGateway != ShouldCreateGateway(ing) {
				t.Errorf("got %v, want %v", !c.shouldCreateGateway, c.shouldCreateGateway)
			}
		})
	}
}
