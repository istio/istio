...
func buildExpectedControllerView(ns, domainSuffix string) *model.ControllerView {
    modelSvcs := []*model.Service { 
        &model.Service{
            Hostname: testService + "." + ns + ".svc." + domainSuffix,
            Ports: model.PortList{ &model.Port{ Name: "http-example", Port: 80, Protocol: model.ProtocolTCP, AuthenticationPolicy: meshconfig.AuthenticationPolicy_INHERIT}, },
...
func TestController(t *testing.T) {
	...
	mockHandler.AssertControllerOK(t, controller, buildExpectedControllerView(ns, domainSuffix))
}
