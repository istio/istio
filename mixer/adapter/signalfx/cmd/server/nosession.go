...
import (
	...
	config "istio.io/istio/mixer/adapter/signalfx/config"
...
// NewSignalfxNoSessionServer creates a new no session server based on given args.
func NewSignalfxNoSessionServer(addr uint16, poolSize int, c *Cert) (*NoSession, error) {
	saddr := fmt.Sprintf(":%d", addr)

	gp := pool.NewGoroutinePool(poolSize, false)
	inf := signalfx.GetInfo()
	s := &NoSession{
		builder:    inf.NewBuilder(),
		env:        handler.NewEnv(0, "signalfx-nosession", gp),
