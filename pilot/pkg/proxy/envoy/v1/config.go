...
func buildSidecarListenersClusters(
	...
	if mesh.ProxyHttpPort > 0 {
		...
		httpOutbound := buildOutboundHTTPRoutes(node, proxyInstances, services, config, true)
...
func buildRDSRoute(mesh *meshconfig.MeshConfig, node model.Proxy, routeName string,
	...
	case model.Sidecar:
		...
		httpConfigs = buildOutboundHTTPRoutes(node, proxyInstances, services, config, false)
...
func buildDestinationHTTPRoutes(node model.Proxy, service *model.Service,
	...
	case model.ProtocolHTTP, model.ProtocolHTTP2, model.ProtocolGRPC:
		...
		for _, rule := range rules {
			httpRoutes := BuildHTTPRoutes(config, rule, service, servicePort, proxyInstances, node.Domain, buildCluster, includeSubsets)
...
func buildOutboundHTTPRoutes(node model.Proxy,
	proxyInstances []*model.ServiceInstance, services []*model.Service, config model.IstioConfigStore,
	includeSubsets bool) HTTPRouteConfigs {
	...
	for _, service := range services {
		for _, servicePort := range service.Ports {
			routes := buildDestinationHTTPRoutes(node, service, servicePort, proxyInstances, config, BuildOutboundCluster, includeSubsets)
...
func buildEgressVirtualHost(serviceName string, destination string,
	...
	routes := buildDestinationHTTPRoutes(node, dest, port, proxyInstances, config, BuildOutboundCluster, true)
