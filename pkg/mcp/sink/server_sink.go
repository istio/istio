...
// ServerSource implements the server implementation of the MCP sink service.
type Server struct {
...
// NewServerSource creates a new instance of a MCP sink server.
func NewServer(options *Options, authChecker AuthChecker) *Server {
	s := &Server{
...
func (s *Server) EstablishResourceStream(stream mcp.ResourceSink_EstablishResourceStreamServer) error {
	...
	// TODO - reset resource state for initial development - remove later
	s.sink.ResetResourceVersions()
