...
func TestSecretOptIn(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"opt-in required, disabled label => disabled": {
			...
			secret:       k8ssecret.BuildSecret("test-sa", "istio.test-sa", "disabled-ns", nil, nil, nil, nil, nil, IstioSecretType),
			expectedActions: []ktesting.Action{
		...
		"opt-in required, enabled label => enabled": {
			...
			secret:       k8ssecret.BuildSecret("test-sa", "istio.test-sa", "enabled-ns", nil, nil, nil, nil, nil, IstioSecretType),
			expectedActions: []ktesting.Action{
