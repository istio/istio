...
func stressTestCommon(tb testing.TB, o options) {
	tb.Helper()
...
type options struct {
	...
	maxApplyDelay    time.Duration
	nackRate         float64
	serverIncemental bool
	clientIncRate    float64
...
func (d *driver) initOptions() {
	...
	for i := range d.serverOpts.CollectionOptions {
		...
		co.Incremental = d.options.serverIncemental
	}
