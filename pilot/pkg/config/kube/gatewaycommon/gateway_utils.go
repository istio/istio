...
const (
	// GatewayClassDefaults is a label on ConfigMaps that indicates
	// the ConfigMap contains default values for a specific GatewayClass
	GatewayClassDefaults = "gateway.istio.io/defaults-for-class"

	// GatewayTLSTerminateModeKey is an annotation key for specifying TLS termination mode
	GatewayTLSTerminateModeKey = "gateway.istio.io/tls-terminate-mode"
)

// GetDefaultName returns the default name for a Gateway based on its spec
...
func toNamespaceSet(name string, lbls map[string]string) labels.Set {
	// If namespace label is not set, implicitly insert it to support older Kubernetes versions
	if lbls[corev1.LabelMetadataName] == name {
		// Already set, avoid copies
		return lbls
	}
	// First we need a copy to not modify the underlying object
	ret := make(map[string]string, len(lbls)+1)
	for k, v := range lbls {
		ret[k] = v
	}
	ret[corev1.LabelMetadataName] = name
	return ret
}

// ConvertListenerSetToListener converts a ListenerEntry to a Listener
func ConvertListenerSetToListener(l gatewayx.ListenerEntry) k8s.Listener {
	// For now, structs are identical enough Go can cast them. I doubt this will hold up forever, but we can adjust as needed.
	return k8s.Listener(l)
}
