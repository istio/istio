...
func (c *ConfigWriter) PrintFullSummary(cf ClusterFilter, lf ListenerFilter, rf RouteFilter, epf EndpointFilter, withHeadings bool) error {
	c.pringHeadings("BOOTSTRAP INFO", withHeadings)
	if err := c.PrintBootstrapSummary(); err != nil {
		return err
	}
	_, _ = c.Stdout.Write([]byte("\n"))
	c.pringHeadings("CLUSTER INFO", withHeadings)
	if err := c.PrintClusterSummary(cf); err != nil {
		return err
	}
	_, _ = c.Stdout.Write([]byte("\n"))
	c.pringHeadings("LISTENER INFO", withHeadings)
	if err := c.PrintListenerSummary(lf); err != nil {
		return err
	}
	_, _ = c.Stdout.Write([]byte("\n"))
	c.pringHeadings("ROUTE INFO", withHeadings)
	...
	_, _ = c.Stdout.Write([]byte("\n"))
	c.pringHeadings("ENDPOINTS INFO", withHeadings)
