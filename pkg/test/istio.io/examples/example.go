...
func (example *Example) Run() {
	scopes.CI.Infof("Executing test %s (%d steps)", example.name, len(example.steps))
	//create directory if it doesn't exist
	if _, err := os.Stat("output"); os.IsNotExist(err) {
		err := os.Mkdir("output", os.ModePerm)
		if err != nil {
			scopes.CI.Errorf("test framework failed to create directory: %s\n", err)
			example.t.Fail()
		}
	}
	...
	framework.
		...
		Run(func(ctx framework.TestContext) {
			...
			if !ok {
				scopes.CI.Errorf("test framework unable to get Kubernetes environment\n")
				example.t.Fail()
			}
			...
			for _, step := range example.steps {
				...
				example.t.Logf("%s\n", step)
				if output != "" {
					scopes.CI.Infof("Output: %s\n", output)
				}

				if err != nil {
					scopes.CI.Errorf("Error: %s\n", err.Error())
					example.t.Fail()
				}
			}
		})
