...
Configure requires either the WorkloadGroup artifact path or its location on the API server.`,
  ...
  configure --name foo --namespace bar -o config`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err = createConfig(kubeClient, wg, clusterID, ingressIP, outputDir, cmd.OutOrStderr()); err != nil {
				return err
			}
...
func createConfig(kubeClient kube.ExtendedClient, wg *clientv1alpha3.WorkloadGroup, clusterID, ingressIP,
...
func createCertsTokens(kubeClient kube.ExtendedClient, wg *clientv1alpha3.WorkloadGroup, dir string, out io.Writer) error {
	ns, err := kubeClient.CoreV1().Namespaces().Get(context.Background(), wg.Namespace, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("namespace %s was not found in cluster %s", wg.Namespace, err)
	}
	var caCertNamespaceConfigMapWithRevision string
	revision := ns.Labels["istio.io/rev"]
	if revision == "default" || revision == "" {
		caCertNamespaceConfigMapWithRevision = controller.CACertNamespaceConfigMap
	} else {
		caCertNamespaceConfigMapWithRevision = controller.CACertNamespaceConfigMap + "-" + revision
	}
	rootCert, err := kubeClient.CoreV1().ConfigMaps(wg.Namespace).Get(context.Background(), caCertNamespaceConfigMapWithRevision, metav1.GetOptions{})
