...
var knownTypes = map[string]schemaType{
	...
	model.Experiment.Type: {
		schema: model.Experiment,
		object: &Experiment{
			TypeMeta: meta_v1.TypeMeta{
				Kind:       "Experiment",
				APIVersion: apiVersion(&model.Experiment),
			},
		},
...
// Experiment is the generic Kubernetes API object wrapper
...
// GetSpec from a wrapper
func (in *Experiment) GetSpec() map[string]interface{} {
	return in.Spec
}

// SetSpec for a wrapper
func (in *Experiment) SetSpec(spec map[string]interface{}) {
	in.Spec = spec
}

// GetObjectMeta from a wrapper
func (in *Experiment) GetObjectMeta() meta_v1.ObjectMeta {
	return in.ObjectMeta
}

// SetObjectMeta for a wrapper
func (in *Experiment) SetObjectMeta(metadata meta_v1.ObjectMeta) {
	in.ObjectMeta = metadata
}

// ExperimentList is the generic Kubernetes API list wrapper
...
// GetItems from a wrapper
func (in *ExperimentList) GetItems() []IstioObject {
	out := make([]IstioObject, len(in.Items))
	for i := range in.Items {
		out[i] = &in.Items[i]
	}
