...
1. launch a code space
1. checkout a feature branch from master
1. `export TAG=istio-testing`
1. `export HUB=localhost:5000`
1. `export DOCKER_TARGETS='docker.pilot docker.proxyv2 docker.ztunnel docker.install-cni'`
1. `export DOCKER_BUILD_VARIANTS="distroless"`
...
istioctl waypoint status -n server --waypoint-timeout=5s
# observe the problem
```
...
# test istioctl
istioctl waypoint status -n server --waypoint-timeout=5s --my-fixed-flag=my-flag-setting
```