...
type EndpointAffectingSidecarScope interface {
	// Service dependency
...
// EndpointAffectingConfigs defines the set of configs that can impact endpoint generation
// If new methods or config dependencies are needed, they should be reflected in EndpointAffectingConfig
...
// EndpointAffectingConfig defines the set of configs that can impact endpoint generation.
// This is used to determine whether a config update should trigger an endpoint update.
// These configs are derived from EndpointAffectingConfigs
func EndpointAffectingConfig(k kind.Kind) bool {
	switch k {
	case kind.Endpoints, kind.ServiceEntry, kind.DestinationRule, kind.PeerAuthentication:
		return true
	// EnvoyFilter affects endpoint generation indirectly, not because we depend
	// on the EnvoyFilter config itself for generation, but because EnvoyFilter can modify the cluster in-place
	// and this requires us to trigger a push for the cluster's endpoints.
...
type EndpointBuilder struct {
	...
	sidecarScope EndpointAffectingSidecarScope
...
func NewCDSEndpointBuilder(
	proxy model.ProxyInfo, sidecarScope EndpointAffectingSidecarScope,
