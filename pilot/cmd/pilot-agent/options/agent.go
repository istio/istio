...
func NewAgentOptions(proxy *model.Proxy, cfg *meshconfig.ProxyConfig) *istioagent.AgentOptions {
	o := &istioagent.AgentOptions{
		...
		WASMModuleExpiry:            wasmModuleExpiry,
		WASMPurgeInterval:           wasmPurgeInterval,
		WASMHTTPRequestTimeout:      wasmHTTPRequestTimeout,
		WASMHTTPRequestMaxRetries:   wasmHTTPRequestMaxRetries,
