...
func TestNewCAClient(t *testing.T) {
	testCases := map[string]struct {
		...
		"Not supported": {
			provider:    "random",
			expectedErr: "CA provider \"random\" isn't supported. Currently Istio supports \"GoogleCA,Citadel,Spire,VaultCA\"",
		},
		"Google CA": {
	...
	for id, tc := range testCases {
		...
		if tc.expectedErr == "" {
			...
			if err == nil {
				t.Errorf("Test case [%s]: Expect error message %q, but actually succeeded",
					id, tc.expectedErr)
			} else if err.Error() != tc.expectedErr {
				t.Errorf("Test case [%s]: Expect error message %q, but got %q",
					id, tc.expectedErr, err.Error())
			}
		}
	}
}
