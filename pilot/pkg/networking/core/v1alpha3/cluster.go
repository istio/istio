...
func (configgen *ConfigGeneratorImpl) BuildClusters(proxy *model.Proxy, push *model.PushContext) []*cluster.Cluster {
	...
	case model.SidecarProxy:
		...
		outboundPatcher := clusterPatcher{efw: envoyFilterPatches, pctx: networking.EnvoyFilter_SIDECAR_OUTBOUND, proxy: proxy}
		...
		inboundPatcher := clusterPatcher{efw: envoyFilterPatches, pctx: networking.EnvoyFilter_SIDECAR_INBOUND, proxy: proxy}
	...
	default: // Gateways
		patcher := clusterPatcher{efw: envoyFilterPatches, pctx: networking.EnvoyFilter_GATEWAY, proxy: proxy}
...
func (p clusterPatcher) conditionallyAppend(l []*cluster.Cluster, hosts []host.Name, clusters ...*cluster.Cluster) []*cluster.Cluster {
	...
	for _, c := range clusters {
		if envoyfilter.ShouldKeepCluster(p.pctx, p.efw, c, hosts) {
			pc, applied = envoyfilter.ApplyClusterMerge(p.pctx, p.proxy, p.efw, c, hosts)
			l = append(l, pc)
		}
	}
	if applied {
...
func (p clusterPatcher) incrementFilterMetrics() {
	if p.efw != nil && len(p.efw.Patches[networking.EnvoyFilter_CLUSTER]) != p.patchesApplied {
		...
		for skipped >= 0 {
			envoyfilter.IncrementSkippedMetric("cluster")
			skipped--
		}
	}
}
