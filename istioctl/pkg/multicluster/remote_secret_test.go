...
`, clusterNameAnnotationKey)
	...
	for i := range cases {
		c := &cases[i]
		t.Run(fmt.Sprintf("[%v] %v", i, c.testName), func(t *testing.T) {
			...
			remoteSecret, err := CreateRemoteSecret(opts.ClusterName, opts.Namespace, opts.ServiceAccountName,
				opts.AuthType, opts.AuthPluginName, opts.AuthPluginConfig, client) // TODO
			got := ""
			if err == nil {
				got, err = EncodeRemoteSecret(remoteSecret)
			}
			if c.wantErrStr != "" {
				if err == nil {
