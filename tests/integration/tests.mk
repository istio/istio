...
test.integration.%.analyze: | $(JUNIT_REPORT) check-go-tag
	$(RUN_TEST) ./tests/integration/$(subst .,/,$*)/... -timeout 90m \
...
test.integration-fuzz.%.kube: | $(JUNIT_REPORT) check-go-tag
	$(GO) test -p 1 -vet=off ${T} -tags="integfuzz integ" ./tests/integration/$(subst .,/,$*)/... -timeout 90m \
	${_INTEGRATION_TEST_FLAGS} ${_INTEGRATION_TEST_SELECT_FLAGS} \
	2>&1 | tee >($(JUNIT_REPORT) > $(JUNIT_OUT))
...
test.integration.kube.presubmit: | $(JUNIT_REPORT) check-go-tag
	$(RUN_TEST) ./tests/integration/... -timeout 90m \
	${_INTEGRATION_TEST_FLAGS} ${_INTEGRATION_TEST_SELECT_FLAGS} \
	2>&1 | tee >($(JUNIT_REPORT) > $(JUNIT_OUT))
...
ifeq (${JOB_TYPE},postsubmit)
	...
	$(RUN_TEST) ./tests/integration/security/ ./tests/integration/pilot -timeout 90m \
	${_INTEGRATION_TEST_FLAGS} ${_INTEGRATION_TEST_SELECT_FLAGS} \
