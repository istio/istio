...
func runSelfSignedCATests() error {
	...
	if error := opts.clientset.CoreV1().Secrets(opts.namespace).Delete("istio.default", do); error != nil {
		return error
	}
	glog.Info(`Secret "istio.default" has been deleted`)

	// Test that the deleted secret is re-created properly.
	if _, error := utils.WaitForSecretExist(opts.clientset, opts.namespace, "istio.default",
		secretWaitTime); error != nil {
		return error
	}
