...
echo "Set up kube config file in VM"
vagrant ssh -c "mkdir -p ~/.kube/"
vagrant ssh -c "cp /etc/kubeconfig.yml ~/.kube/config"

echo "Install socat"
vagrang ssh -c "sudo apt-get update && sudo apt-get install socat"
...
vagrant ssh -c "sudo sed -i 's/ExecStart=\/usr\/bin\/dockerd -H fd:\/\//ExecStart=\/usr\/bin\/dockerd -H fd:\/\/ --insecure-registry 10.0.0.0\/8/' /lib/systemd/system/docker.service"
