...
func newConnection(stream DiscoveryStream) *Connection {
	return &Connection{
		pushChannel:   make(chan *Event),
		stop:          make(chan struct{}),
		PeerAddr:      getPeerAddress(stream),
		Connect:       time.Now(),
		stream:        stream,
		blockedPushes: map[string]*model.PushRequest{},
	}
...
func (s *DiscoveryServer) processRequest(req *discovery.DiscoveryRequest, con *Connection) error {
	...
	shouldRespond := s.StreamHandler.ShouldRespond(con, req)
...
func (s *DiscoveryServer) InitializeStream(con *Connection) error {
	...
	peerAddr := getPeerAddress(con.stream)

	ids, err := s.authenticate(con.stream.Context())
	if err != nil {
		return err
	}
	con.Identities = ids
...
func (s *DiscoveryServer) StreamAggregatedResources(stream discovery.AggregatedDiscoveryService_StreamAggregatedResourcesServer) error {
	return s.StreamHandler.Stream(stream)
}
