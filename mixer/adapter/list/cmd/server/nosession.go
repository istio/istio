...
import (
	...
	listchecker "istio.io/istio/mixer/adapter/list"
	config "istio.io/istio/mixer/adapter/list/config"
...
// NewListcheckerNoSessionServer creates a new no session server based on given args.
func NewListcheckerNoSessionServer(addr uint16, poolSize int, c *Cert) (*NoSession, error) {
	saddr := fmt.Sprintf(":%d", addr)

	gp := pool.NewGoroutinePool(poolSize, false)
	inf := listchecker.GetInfo()
	s := &NoSession{
		builder:    inf.NewBuilder(),
		env:        handler.NewEnv(0, "listchecker-nosession", gp),
