...
func (conn *BaseConnection) Peer() string {
	return conn.peerAddr
}

func (conn *BaseConnection) SetID(id string) {
	conn.conID = id
}

func (conn *BaseConnection) DoneInitialize() {
...
func (conn *BaseConnection) StreamClosed() <-chan struct{} {
	if conn.stream != nil {
...
func NewConnection(peerAddr string, stream DiscoveryStream) BaseConnection {
...
func Send(ctx ConnectionContext, res *discovery.DiscoveryResponse) error {
	...
	if err == nil {
		...
		// XXX xdsResponseWriteTimeouts.Increment()
	}
	return err
}
...
func ShouldRespond(w Watcher, id string, request *discovery.DiscoveryRequest) (bool, ResourceDelta) {
	...
	if request.ErrorDetail != nil {
		...
		// XXX incrementXDSRejects(request.TypeUrl, w.GetID(), errCode.String())
	...
	if request.ResponseNonce != previousInfo.NonceSent {
		...
		// XXX: xdsExpiredNonce.With(typeTag.Value(v3.GetMetricType(request.TypeUrl))).Increment()
		return false, emptyResourceDelta
	}
...
func Receive(ctx ConnectionContext) {
	...
	for {
		...
		if err != nil {
			...
			// XXX: totalXDSInternalErrors.Increment()
			return
		}
