...
func TestBasicStartStop(t *testing.T) {
	...
	c.Start(rt.NewProvider(k, metav1.NamespaceAll, 0), basicmeta2.MustGet().KubeCollections().All())
	defer c.Stop()
...
func TestNoReconcilation(t *testing.T) {
	...
	c.Start(rt.NewProvider(k, metav1.NamespaceAll, 0), basicmeta2.MustGet().KubeCollections().All())
	c.UpdateResourceStatus(basicmeta2.K8SCollection1.Name(), resource.NewFullName("foo", "bar"), "v1", "s1")
	defer c.Stop()
...
func TestBasicReconcilation_BeforeUpdate(t *testing.T) {
	...
	c.Start(rt.NewProvider(k, metav1.NamespaceAll, 0), basicmeta2.MustGet().KubeCollections().All())
	c.UpdateResourceStatus(basicmeta2.K8SCollection1.Name(), resource.NewFullName("foo", "bar"), "v1", s)
	c.Report(diag.Messages{})
	defer c.Stop()
...
func TestBasicReconcilation_AfterUpdate(t *testing.T) {
	...
	c.UpdateResourceStatus(
		basicmeta2.K8SCollection1.Name(), resource.NewFullName("foo", "bar"), "v1", s)
	defer c.Stop()
