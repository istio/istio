...
func TestDispatcher_Handle(t *testing.T) {
	...
	fh2 := &fakeHandler{}
	fh3 := &fakeHandler{}
	b.Add(emptyInfo.TypeURL, fh1)
	b.Add(emptyInfo.TypeURL, fh2)
	b.Add(structInfo.TypeURL, fh3)
	d := b.Build()

	_ = d.Handle(addRes1V1())
	_ = d.Handle(addRes2V1())
	if len(fh1.evts) != 2 {
...
func TestDispatcher_UnhandledEvent(t *testing.T) {
	...
	changed := d.Handle(addRes1V1())
	if changed {
		t.Fatal("Shouldn't have changed")
	}
}
...
func (f *fakeHandler) Handle(e resource.Event) bool {
