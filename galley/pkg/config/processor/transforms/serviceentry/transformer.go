...
func (t *transformer) Start(o interface{}) {
	options := o.(processing.ProcessorOptions)
	...
	t.converter = converter.New(options.DomainSuffix, podCache)
...
func (t *transformer) Handle(e event.Event) {
	scope.Errorf("serviceentry.transformer: ==> %v", e)
	switch e.Kind {
	case event.FullSync:
		t.fullSyncCtr--
		if t.fullSyncCtr == 0 {
...
// func (t *transformer) updateVersion() {
// 	// TODO: monitoring.RecordStateTypeCountWithContext(h.statsCtx, len(h.mcpResources))
//
