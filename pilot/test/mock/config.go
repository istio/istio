...
var (
	...
	ExampleRouteRule = &routerule.RouteRule{
	...
	ExampleIngressRule = &routerule.IngressRule{
		Destination: &routerule.IstioService{
			Name: "world",
		},
		Port: 80,
		DestinationServicePort: &routerule.IngressRule_DestinationPort{DestinationPort: 80},
	}

	// ExampleEgressRule is an example egress rule
	ExampleEgressRule = &routerule.EgressRule{
		Destination: &routerule.IstioService{
			Service: "*cnn.com",
		},
		Ports:          []*routerule.EgressRule_Port{{Port: 80, Protocol: "http"}},
		UseEgressProxy: false,
	}

	// ExampleDestinationPolicy is an example destination policy
	ExampleDestinationPolicy = &routerule.DestinationPolicy{
		Destination: &routerule.IstioService{
			Name: "world",
		},
		LoadBalancing: &routerule.LoadBalancing{
			LbPolicy: &routerule.LoadBalancing_Name{Name: routerule.LoadBalancing_RANDOM},
		},
	}
