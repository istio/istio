...
def searchpod(pi, searchstr):
  ...
  if "." in searchstr:
    ...
    if len(si) != 3 and len(si) != 2:
      logging.error(
          "podname must be either name.namespace.podip or name.namespace or "
          "any string that's a pod's label or a prefix of a pod's name")
      return None
...
def start_port_forward(pod_name, namespace, remote_port):
  ...
  url = ""
  try:
    port_forward_pid = subprocess.Popen(
        "kubectl --namespace={namespace} port-forward {pod_name} "
        "{local_port}:{remote_port}".format(
            pod_name=pod_name,
            namespace=namespace,
            local_port=local_port,
            remote_port=remote_port).split(),
        stdout=open(os.devnull, "wb")).pid
  except:
    logging.error(
        "Failed to create port-forward for pod %s.%s with remote port %s",
...
def find_pilot_url():
  try:
    pilot_svc = subprocess.check_output(
        "kubectl get svc {svc} -n {ns} -o json".format(
            svc=PILOT_SVC, ns=ISTIO_NS).split())
  except:
    pilot_svc = {}
  pilot_url = ""
  pilot_port = ""
  ...
  if pilot_svc:
    ...
    try:
      ...
      cmd = ("kubectl --namespace=%s get -l istio=pilot pod "
             "-o=jsonpath={.items[0].metadata.name}") % (
                 ISTIO_NS)
      pod_name = subprocess.check_output(cmd.split())
      pilot_url, port_forward_pid = start_port_forward(pod_name, ISTIO_NS,
                                                       pilot_port)
...
def main(args):
  ...
  if not args.skip_pilot:
    ...
    if pilot_url:
      if not pilot_url.startswith("http://") and not pilot_url.startswith(
          "https://"):
    ...
    logging.info("Fetching from Pilot for pod %s in %s namespace", pod.name,
                 pod.namespace)
    xds = XDS(url=pilot_url)
    data = xds.lds(pod, True)
    yaml.safe_dump(
        data, op, default_flow_style=False, allow_unicode=False, indent=2)
    print "Wrote ", output_file
    if args.show_ssl_summary:
        for l in data["listeners"]:
            state = "SSL" if "ssl_context" in l else "PLAINTEXT"
            print ("Listener {0:30s} : {1:10s}".format(l["name"], state))
  ...
  if not args.skip_envoy:
    ...
    logging.info("Fetching from Envoy for pod %s in %s namespace", pod.name,
                 pod.namespace)
    pr = Proxy(envoy_url)
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--pilot_url",
      help=
      "Often this is localhost:8080 or 15005 (https) or 15007 (http) through a port-forward."
  ...
  parser.add_argument(
      "podname",
      help=
      "podname must be either name.namespace.podip or name.namespace or any string that is a pod's label or a prefix of a pod's name. ingress, mixer, istio-ca, product-page all work"
  )
  parser.add_argument(
      "--output",
      help=
      "A directory where output files are saved. default is the /tmp directory"
  )
  parser.add_argument(
      "--skip_envoy",
      action="store_true",
      help="Fetch Envoy configuration from a pod")
  parser.add_argument(
      "--skip_pilot",
      action="store_true",
      help="Fetch from pilot Proxy configuration for a pod")
  parser.add_argument(
      "--show_ssl_summary",
      action="store_true",
