...
func TestRejectConflictingEgressRules(t *testing.T) {
	cases := []struct {
		name  string
		in    map[string]*routerule.EgressRule
		out   map[string]*routerule.EgressRule
		valid bool
	}{
		{name: "no conflicts",
			in: map[string]*routerule.EgressRule{"cnn": {
			...
			out: map[string]*routerule.EgressRule{"cnn": {
				Destination: &routerule.IstioService{
					Service: "*cnn.com",
				},
				Ports: []*routerule.EgressRule_Port{
		...
		{name: "a conflict in a domain",
			in: map[string]*routerule.EgressRule{"cnn2": {
				Destination: &routerule.IstioService{
					Service: "*cnn.com",
				},
				Ports: []*routerule.EgressRule_Port{
