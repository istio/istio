...
var (
	...
	// Description: Port name is not under naming convention. Proxy could potentially prevent TCP traffic on the same port from being forwarded correctly
	ExternalNameServiceTypeInvalidPortName = diag.NewMessageType(diag.Warning, "IST0150", "Port name %s (port: %d, targetPort: %s) doesn't follow the naming convention of Istio port. Proxy could potentially prevent TCP traffic on the same port from being forwarded correctly")
)
