		...
		for i, rule := range in.GetRules() {
			...
			for _, from := range rule.From {
				...
				if from.Source == nil {
					...
					errs = appendErrors(errs, security.CheckWildcardValues("TrustDomains", src.TrustDomains))
					...
					errs = appendErrors(errs, security.CheckWildcardValues("NotTrustDomains", src.NotTrustDomains))
