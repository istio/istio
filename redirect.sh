...
$IPTABLES -t mangle -A uproxy-PREROUTING -p tcp -i "${INTERFACE_PREFIX}+" ! --dport 15088 -m set --match-set uproxy-pods-ips dst -j TPROXY --tproxy-mark $MARK --on-port $POD_INBOUND --on-ip 127.0.0.1

# external traffic to pod
# tproxy connections coming from outside the node to "injected" pods.
$IPTABLES -t mangle -A uproxy-PREROUTING -p tcp -i "${INCOMING_INTERFACE}" ! --dport 15088 -m set --match-set uproxy-pods-ips dst -j TPROXY --tproxy-mark $MARK --on-port $POD_INBOUND --on-ip 127.0.0.1
