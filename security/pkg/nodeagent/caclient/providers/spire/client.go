...
func (c *spireClient) CSRSign(ctx context.Context, csrPEM []byte, token string,
	...
	certChain, err := c.buildCertChain(ctx, certificates, bundles)
	if err != nil {
		return nil, err
	}
...
func (c *spireClient) buildCertChain(ctx context.Context, certificates, bundles []*x509.Certificate) ([]string, error) {
