...
func TestBuildGatewayListenerTlsContext(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "ecdh curves specified with tls SIMPLE",
			server: &networking.Server{
				Hosts: []string{"httpbin.example.com", "bookinfo.example.com"},
				Port: &networking.Port{
					Protocol: string(protocol.HTTPS),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ret := cgi.createGatewayHTTPFilterChainOpts(tc.node, tc.server.Port, tc.server,
				tc.routeName, tc.proxyConfig, tc.transportProtocol, cg.PushContext(), types.NamespacedName{})
...
// stubGatewayController satisfies pilot_model.GatewayController for tests that don't otherwise
// need one.
type stubGatewayController struct {
	pilot_model.ConfigStoreController
}

func (stubGatewayController) Reconcile(*pilot_model.PushContext)                         {}
func (stubGatewayController) SecretAllowed(config.GroupVersionKind, string, string) bool { return true }

func (stubGatewayController) HasInferencePool(types.NamespacedName) bool { return false }

// TestBuildGatewayListenersAuthzListenerSetScoping runs a Gateway plus a synthetic ListenerSet-derived
// Server (mirroring what pilot/pkg/config/kube/gateway/gateway_collection.go produces) through the real
// merge + listener-building pipeline, with an AuthorizationPolicy targeting only the ListenerSet. It
// verifies the resulting RBAC filter appears on the ListenerSet's own port and not on the Gateway's.
func TestBuildGatewayListenersAuthzListenerSetScoping(t *testing.T) {
	configs := []config.Config{
		{
			Meta: config.Meta{Name: "gateway", Namespace: "testns", GroupVersionKind: gvk.Gateway},
			Spec: &networking.Gateway{
				Servers: []*networking.Server{
					{
						Port:  &networking.Port{Name: "https", Number: 443, Protocol: string(protocol.HTTPS)},
		...
		{
			Meta: config.Meta{
				Name:             "gateway~istio-autogenerated-k8s-gateway~https",
				Namespace:        "testns",
				GroupVersionKind: gvk.Gateway,
				Annotations: map[string]string{
					constants.InternalParentNames: "ListenerSet/my-ls/https.testns",
				},
			},
			Spec: &networking.Gateway{
				Servers: []*networking.Server{
					{
						Port:  &networking.Port{Name: "https", Number: 8443, Protocol: string(protocol.HTTPS)},
		...
		{
			Meta: config.Meta{Name: "ls-policy", Namespace: "testns", GroupVersionKind: gvk.AuthorizationPolicy},
			Spec: &security.AuthorizationPolicy{
				...
				Rules: []*security.Rule{{To: []*security.Rule_To{{Operation: &security.Operation{Methods: []string{"GET"}}}}}},
			},
		},
	}

	cg := NewConfigGenTest(t, TestOptions{Configs: configs})
	// buildHTTPConnectionManager consults GatewayAPIController for InferencePools whenever the proxy
	// carries the k8s gateway-name label; NewConfigGenTest doesn't wire up a real one, so stub it.
	cg.PushContext().GatewayAPIController = stubGatewayController{}
	proxy := cg.SetupProxy(&pilot_model.Proxy{
		Type:            pilot_model.Router,
		ConfigNamespace: "testns",
		Labels:          map[string]string{label.IoK8sNetworkingGatewayGatewayName.Name: "gateway"},
		Metadata:        &pilot_model.NodeMetadata{Labels: map[string]string{label.IoK8sNetworkingGatewayGatewayName.Name: "gateway"}},
	})

	lb := NewListenerBuilder(proxy, cg.PushContext())
	builder := cg.ConfigGen.buildGatewayListeners(lb)

	hasRBAC := func(listenerName string) bool {
		l := xdstest.ExtractListener(listenerName, builder.gatewayListeners)
		if l == nil {
			t.Fatalf("listener %s not found", listenerName)
		}
		if len(l.FilterChains) != 1 {
			t.Fatalf("listener %s: expected 1 filter chain, got %d", listenerName, len(l.FilterChains))
		}
		_, httpFilters := xdstest.ExtractFilterNames(t, l.FilterChains[0])
		return slices.Contains(httpFilters, wellknown.HTTPRoleBasedAccessControl)
	}

	assert.Equal(t, hasRBAC("0.0.0.0_443"), false, "Gateway's own listener should not get the ListenerSet-scoped policy")
	assert.Equal(t, hasRBAC("0.0.0.0_8443"), true, "ListenerSet's listener should get the ListenerSet-scoped policy")
}

// TestBuildGatewayListenersAuthzListenerSetScopingPlainHTTP is the plain-HTTP analog of
// TestBuildGatewayListenersAuthzListenerSetScoping. Plain HTTP servers have no SNI, so scoping here
// depends on commonListenerSetScope agreeing across all servers sharing a port; this covers both the
// straightforward case (each port has only one origin) and the ambiguous fallback (a port shared by
// servers from different origins, where scoping can't be resolved and falls back to gateway-wide).
func TestBuildGatewayListenersAuthzListenerSetScopingPlainHTTP(t *testing.T) {
	configs := []config.Config{
		{
			Meta: config.Meta{Name: "gateway", Namespace: "testns", GroupVersionKind: gvk.Gateway},
			Spec: &networking.Gateway{
				Servers: []*networking.Server{
					{
						Port:  &networking.Port{Name: "http", Number: 8080, Protocol: string(protocol.HTTP)},
						Hosts: []string{"a.example.com"},
					},
					{
						Port:  &networking.Port{Name: "shared", Number: 9090, Protocol: string(protocol.HTTP)},
						Hosts: []string{"shared-a.example.com"},
					},
				},
			},
		},
		{
			Meta: config.Meta{
				...
				GroupVersionKind: gvk.Gateway,
				Annotations: map[string]string{
					constants.InternalParentNames: "ListenerSet/my-ls/http.testns",
				},
			},
			Spec: &networking.Gateway{
				Servers: []*networking.Server{
					{
						Port:  &networking.Port{Name: "http", Number: 8081, Protocol: string(protocol.HTTP)},
						Hosts: []string{"b.example.com"},
					},
					{
						Port:  &networking.Port{Name: "shared", Number: 9090, Protocol: string(protocol.HTTP)},
						Hosts: []string{"shared-b.example.com"},
					},
				},
			},
		},
	...
	assert.Equal(t, hasRBAC("0.0.0.0_8080"), false, "Gateway's own unshared port should not get the ListenerSet-scoped policy")
	assert.Equal(t, hasRBAC("0.0.0.0_8081"), true, "ListenerSet's own unshared port should get the ListenerSet-scoped policy")
	assert.Equal(t, hasRBAC("0.0.0.0_9090"), false,
		"port shared by both origins is ambiguous and must fall back to unscoped (absent, since neither policy targets the Gateway itself here)")
}
