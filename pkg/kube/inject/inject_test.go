...
// TestZoneAwareLBInjectsEnvVar verifies that when istiod is configured with
// ENABLE_ZONE_AWARE_LOAD_BALANCER=true, the sidecar injection template emits the
// matching env-var into the istio-proxy container so pilot-agent renders the
// bootstrap with local_cluster. The env-var must NOT appear when the flag is off.
func TestZoneAwareLBInjectsEnvVar(t *testing.T) {
	const envName = "ENABLE_ZONE_AWARE_LOAD_BALANCER"
	...
	cases := []struct {
		...
		{"flag on injects env-var", true, true},
		{"flag off omits env-var", false, false},
	}

	sidecarTemplate, valuesConfig, mc := getInjectionSettings(t, nil, "")
	inputYAML, err := os.ReadFile("testdata/inject/hello.yaml")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			test.SetForTest(t, &features.EnableZoneAwareLB, tc.flagValue)

			env := &model.Environment{}
			env.SetPushContext(&model.PushContext{ProxyConfigs: &model.ProxyConfigs{}})
			webhook := &Webhook{
				Config:       sidecarTemplate,
				meshConfig:   mc,
				env:          env,
			...
			if got.Patch == nil {
				t.Fatal("expected patch from injection, got nil")
			}
			patch := prettyJSON(got.Patch, t)
			patchedJSON := applyJSONPatch(templateJSON, patch, t)
			patchedPod := &corev1.Pod{}
			...
			for i, c := range patchedPod.Spec.Containers {
				if c.Name == "istio-proxy" {
					proxy = &patchedPod.Spec.Containers[i]
					break
				}
			}
			if proxy == nil {
				t.Fatal("istio-proxy container not found in injected pod")
			}
			hasEnv := false
			for _, e := range proxy.Env {
				if e.Name == envName {
					hasEnv = true
					if e.Value != "true" {
						t.Errorf("%s = %q, want %q", envName, e.Value, "true")
					}
					break
				}
			}
			if hasEnv != tc.wantEnv {
				t.Errorf("env-var %s present=%v, want %v", envName, hasEnv, tc.wantEnv)
			}
		})
	}
}
