...
func getGatewayClasses(r GatewayResources) map[string]k8s.GatewayController {
	...
	for _, obj := range r.GatewayClass {
		...
		classInfo, known := classInfos[gwc.ControllerName]
		if !known {
			continue
		}
		...
		obj.Status.(*kstatus.WrappedStatus).Mutate(func(s config.Status) config.Status {
			...
			*gcs = GetClassStatus(gcs, obj.Generation)
			gcs.Routabilities = classInfo.routabilities
			return gcs
		})
	}
...
func reportGatewayStatus(
	...
	obj.Status.(*kstatus.WrappedStatus).Mutate(func(s config.Status) config.Status {
		...
		for _, addr := range addressesToReport {
			route := k8sbeta.GatewayRoutabilityCluster
			if addrType == k8s.IPAddressType {
				route = k8sbeta.GatewayRoutabilityPublic
			}
...
func extractGatewayServices(r GatewayResources, kgw *k8s.GatewaySpec, obj config.Config, classInfo classInfo) ([]string, *ConfigError) {
	supportedRoutability := classInfo.routabilities
	if kgw.Infrastructure != nil && kgw.Infrastructure.Routability != nil {
		if !slices.Contains(supportedRoutability, *kgw.Infrastructure.Routability) {
			return nil, &ConfigError{
				Reason:  InvalidRouteability,
				Message: fmt.Sprintf("unsupported routability %q, want one of %v", *kgw.Infrastructure.Routability, supportedRoutability),
			}
		}
	}
	if IsManaged(kgw) {
