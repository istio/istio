...
func (m *NftablesTrafficManager) CreateInpodRules(log *istiolog.Scope, podOverrides config.PodLevelOverrides, _ string) error {
...
func (m *NftablesTrafficManager) DeleteInpodRules(log *istiolog.Scope, _ string) error {
	if m.podNftables == nil {
		return fmt.Errorf("pod nftables configurator not available (this is likely a host-only traffic manager)")
	}
