...
func TestMetricRotationInterval(t *testing.T) {
	cases := []struct {
		name     string
		proxy    *Proxy
		...
		},
		{
			...
			proxy: &Proxy{
				Metadata: &NodeMetadata{
					ProxyConfig: &NodeMetaProxyConfig{
						ProxyMetadata: map[string]string{
							"ISTIO_META_METRIC_ROTATION_INTERVAL": "10",
						},
					},
				},
			},
		...
		},
		{
			...
			proxy: &Proxy{
				Metadata: &NodeMetadata{
					ProxyConfig: &NodeMetaProxyConfig{
						ProxyMetadata: map[string]string{
							"ISTIO_META_METRIC_ROTATION_INTERVAL": "10s",
						},
					},
				},
			},
			expected: durationpb.New(10 * time.Second),
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := metricRotationInterval(tc.proxy)
			assert.Equal(t, tc.expected, got)
		})
	}
}

func TestMetricGracefulDeletionInterval(t *testing.T) {
	cases := []struct {
		name     string
		proxy    *Proxy
		expected *durationpb.Duration
		...
		},
		{
			...
			proxy: &Proxy{
				Metadata: &NodeMetadata{
					ProxyConfig: &NodeMetaProxyConfig{
						ProxyMetadata: map[string]string{
							"ISTIO_META_METRIC_GRACEFUL_DELETION_INTERVAL": "10s",
						},
					},
				},
			},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := metricGracefulDeletionInterval(tc.proxy)
			assert.Equal(t, tc.expected, got)
		})
	}
}
