...
func buildIstioCNIPlugin(cfg *config.InstallConfig) ([]byte, error) {
	...
	return json.MarshalIndent(pluginConfig, "", "  ")
}
...
// unmarshalCNIConfig parses CNI config bytes into a map. kind is used in the error
// message to identify which config failed to parse (e.g. "existing", "Istio").
func unmarshalCNIConfig(cniConfig []byte, kind string) (map[string]any, error) {
	var cniConfigMap map[string]any
	if err := json.Unmarshal(cniConfig, &cniConfigMap); err != nil {
		return nil, fmt.Errorf("error loading %s CNI config (JSON error): %v", kind, err)
	}
