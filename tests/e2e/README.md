...
# Running E2E tests on your local kubernets cluster

* [Step 1: Run kubernetes cluster using minikube locally](#step-1-Run-kubernetes-cluster-using-minikube-locally)
* [Step 2: Setup local registry](#step-2-setup-local-registry)
* [Step 3: Run](#step-3-run)

## Step 1: Run kubernetes cluster using minikube locally
Details on set up are available [here](https://kubernetes.io/docs/getting-started-guides/minikube/).
...
Setting up cluster:
If you are using kubernetes with version >= v1.8.0 for kubeadm, the extra-config and RBAC flags will be enabled by default. Explicitly enabling them will actually cause errors. Please run:

```bash
minikube start --bootstrapper kubeadm
```

If you are using kubernetes with version < v1.8.0, you need to specify the extra-config and RBAC flags explicitly. Please run:
...
You can follow steps [here](../util/localregistry/README.md) to setup local docker registry. This will enable you to push your local changes to local registry which will get forwarded to local docker registry on minikube.


## Step 3: Run
Setup HUB and TAG as follows:
```bash
