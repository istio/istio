...
func TestResolveAddr(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			errStr:   "ParseAddr(\"localhost\"): unable to parse IP",
			lookup:   nil,
		},
		{
		...
		},
		{
			...
			expected: "",
			errStr:   "ParseAddr(\"127.0.0.1.1\"): IPv4 address too long",
			lookup:   nil,
		},
		{
			name:     "Bad IPv6",
			...
			expected: "",
			errStr:   "ParseAddr(\"2001:db8::bad::1\"): multiple :: in address (at \":1\")",
			lookup:   nil,
		},
		{
			name:     "Empty host",
		...
		},
		{
			...
			expected: "",
			errStr:   "invalid ip:port \"2001:db8::20:9080\", IPv6 addresses must be surrounded by square brackets",
			lookup:   nil,
		},
		{
