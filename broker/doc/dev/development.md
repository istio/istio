...
supported toolchain versions for building Broker. If you find a
...
[submit an issue](https://github.com/istio/broker/issues/new).

This document is intended to be relative to the branch in which it is found.
It is guaranteed that requirements will change over time for the development
branch, but release branches of Broker should not change.
...
Broker's code base has only a few external dependencies you
need to setup before being able to build and run the code.

### Setting up Go

Broker is written in the [Go](http://golang.org) programming language.
To build Broker, you'll need a Go development environment. Builds for
Broker require Go version 1.8. If you haven't set up a Go development
...
Broker is built using the bazel build system. See
...
Broker's code into your GOPATH. Note: the commands below will not work if
...
git clone https://github.com/$GITHUB_USER/broker.git
cd broker
git remote add upstream 'https://github.com/istio/broker.git'
...
Run
```shell
user@host:~/GOHOME/src/istio.io/broker$ ./bin/pre-commit
...
github.com/istio/broker, you should modify your git configuration so
...
1. Visit https://github.com/$GITHUB_USER/broker
...
It will occasionally be necessary to add a new dependency to Broker, either in support of a new adapter or to provide
additional core functionality.

Broker dependencies are maintained in the [WORKSPACE](https://github.com/istio/broker/blob/master/WORKSPACE)
