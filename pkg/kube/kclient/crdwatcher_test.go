...
func TestCRDWatcherMaximumVersion(t *testing.T) {
	// Temporarily add TLSRoute_v1alpha2 to MaximumCRDVersions at max 1.4.0 (inclusive).
	// This simulates the backport scenario: the Go client targets v1alpha2 and GWAPI > 1.4.0
	// only serves v1, so we must not watch v1alpha2 when bundle version > 1.4.0.
	...
	t.Run("above maximum is rejected", func(t *testing.T) {
		...
		assert.Equal(t, ctl.KnownOrCallback(gvr.TLSRoute, func(_ <-chan struct{}) {}), false)
	})
	...
	t.Run("equal to maximum is accepted", func(t *testing.T) {
		...
		c := kube.NewFakeClient()
		clienttest.MakeCRDWithAnnotations(t, c, gvr.TLSRoute, map[string]string{
			consts.BundleVersionAnnotation: "v1.4.0",
		})
		calls := atomic.NewInt32(0)
		ctl := c.CrdWatcher()
		assert.Equal(t, ctl.KnownOrCallback(gvr.TLSRoute, func(s <-chan struct{}) {
