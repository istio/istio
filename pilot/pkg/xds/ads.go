...
func (s *DiscoveryServer) computeProxyState(proxy *model.Proxy, request *model.PushRequest) {
	...
	if request == nil {
		...
		for conf := range request.ConfigsUpdated {
			switch conf.Kind {
			case kind.ServiceEntry, kind.DestinationRule, kind.VirtualService, kind.Sidecar, kind.HTTPRoute, kind.TCPRoute, kind.TLSRoute, kind.GRPCRoute, kind.BackendLBPolicy:
				shouldResetSidecarScope = true
