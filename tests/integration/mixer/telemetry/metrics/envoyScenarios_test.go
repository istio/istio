...
// Port of TestMetric
func TestIngessToPrometheus_ServiceMetricEnvoy(t *testing.T) {
	framework.
		NewTest(t).
		...
		Run(func(ctx framework.TestContext) {
			...
			testMetricEnvoy(t, ctx, label, labelValue)
		})
}
...
func testMetricEnvoy(t *testing.T, ctx framework.TestContext, label string, labelValue string) { // nolint:interfacer
	t.Helper()
...
func TestMainEnvoy(m *testing.M) {
	framework.
...
components:
  ...
  telemetry:
    enabled: true`
		...
		Setup(testsetupenvoy).
		Run()
}

func testsetupenvoy(ctx resource.Context) (err error) {
