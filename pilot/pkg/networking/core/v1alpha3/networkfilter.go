...
func buildOutboundNetworkFiltersWithWeightedClusters(node *model.Proxy, routes []*networking.RouteDestination,
	...
	for _, route := range routes {
		service := push.ServiceForHostnameAndNamespace(node, host.Name(route.Destination.Host), "")
		if route.Weight > 0 {
...
func buildOutboundNetworkFilters(node *model.Proxy,
	...
	if len(routes) == 1 {
		service := push.ServiceForHostnameAndNamespace(node, host.Name(routes[0].Destination.Host), "")
