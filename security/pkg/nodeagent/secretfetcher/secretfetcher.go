...
const (
	...
	// IngressGatewayFallbackSecret is the name of an environment variable that can be optionally specified in order
	// to supply a name of a Kubernetes Secret that should be used as a fallback in case a secret with a requested name
	// doesn't exist (or hasn't been discovered yet).
	IngressGatewayFallbackSecretEnvKey = "INGRESS_GATEWAY_FALLBACK_SECRET"
)
...
func (sf *SecretFetcher) FindIngressGatewaySecret(key string) (secret model.SecretItem, ok bool) {
	...
	if !exist {
		// Requested secret doesn't exist. A fallback logic may be optionally enabled by supplying an alternative
		// secret name via a corresponding environment variable. In this case a supplied fallback secret name will
		// be attempted if the originally requested one is missing. The fallback should be completely transparent
		// for users of this API to make sure original resource name is preserved and will be picked up if the original
		// resource is created/discovered at a later stage.
		log.Warnf("Requested secret %q doesn't exist or hasn't been discovered yet", key)
		if fallback, ok := os.LookupEnv(IngressGatewayFallbackSecretEnvKey); ok {
			log.Infof("Attempting to use a configured fallback secret %q instead of %q", fallback, key)
			val, exist = sf.secrets.Load(fallback)
			if !exist {
				log.Warnf("Fallback secret %q doesn't exist or hasn't been discovered yet", fallback)
			}
		}
	}
