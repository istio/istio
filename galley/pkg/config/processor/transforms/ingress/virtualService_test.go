...
func TestVirtualService_Input_Output(t *testing.T) {
	...
	g.Expect(xform.Outputs()).To(Equal(collection.Names{metadata.IstioNetworkingV1Alpha3Virtualservices.Name}))
}
...
func TestVirtualService_AddSync(t *testing.T) {
	...
	g.Eventually(acc.Events).Should(ConsistOf(
		event.AddFor(metadata.IstioNetworkingV1Alpha3Virtualservices.Name, vs1()),
		event.FullSyncFor(metadata.IstioNetworkingV1Alpha3Virtualservices.Name)))
}
...
func TestVirtualService_AddUpdateDelete(t *testing.T) {
	...
	g.Eventually(acc.Events).Should(ConsistOf(
		event.FullSyncFor(metadata.IstioNetworkingV1Alpha3Virtualservices.Name),
		event.AddFor(metadata.IstioNetworkingV1Alpha3Virtualservices.Name, vs1()),
		event.UpdateFor(metadata.IstioNetworkingV1Alpha3Virtualservices.Name, vs1v2()),
		event.DeleteFor(metadata.IstioNetworkingV1Alpha3Virtualservices.Name, vs1v2().Metadata.FullName, vs1v2().Metadata.Version),
	))
}
...
func setupVS(g *GomegaWithT, o processing.ProcessorOptions) (event.Transformer, *fixtures.Source, *fixtures.Accumulator) {
	...
	xform := xforms[1]
	src.Dispatch(xform)
	xform.DispatchFor(metadata.IstioNetworkingV1Alpha3Virtualservices.Name, acc)
