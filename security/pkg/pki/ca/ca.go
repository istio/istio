...
const (
	...
	// CASecret stores the key/cert of self-signed CA for persistence purpose.
...
func NewSelfSignedIstioCAOptions(
	...
	caOpts = &IstioCAOptions{
		...
		RotatorConfig: &SelfSignedCARootCertRotatorConfig{
			CheckInterval:      rootCertCheckInterval,
			caCertTTL:          caCertTTL,
	...
	// For the first time the CA is up, if CASecret is not exist,
	...
	if scrtErr != nil {
		...
		caSecret, scrtErr = client.Secrets(namespace).Get(context.TODO(), CASecret, metav1.GetOptions{})
		if scrtErr != nil {
			pkiCaLog.Errorf("Failed to get secret (error: %s), exit", scrtErr)
			return nil, fmt.Errorf("failed to get secret (error: %s)", scrtErr)
		}
	}
