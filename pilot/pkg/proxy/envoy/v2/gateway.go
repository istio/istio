...
func buildGatewayHTTPListeners(mesh *meshconfig.MeshConfig,
	configStore model.IstioConfigStore, node model.Proxy) ([]*api.Listener, error) {
	...
	listeners := make([]*api.Listener, 0, len(gateway.Servers))
...
func buildGatewayVirtualHosts(configStore model.IstioConfigStore, node model.Proxy, listenerPort int) (*v1.HTTPRouteConfig, error) {
	gateways, err := configStore.List(model.Gateway.Type, model.NamespaceAll)
	if err != nil {
		return nil, err
	}
		...
		for _, server := range gateway.Servers {
			...
			for _, externalHostname := range server.Hosts {
				routesForThisVirtualHost, err := buildDestinationHTTPRoutesForGatewayVirtualHost(allRules,
					node, configStore, listenerPort, v1.BuildOutboundCluster,
				...
				virtualHosts = append(virtualHosts, &v1.VirtualHost{
	...
	configs := (&v1.HTTPRouteConfig{VirtualHosts: virtualHosts}).Normalize()
	return configs, nil
}
...
func buildDestinationHTTPRoutesForGatewayVirtualHost(
	...
	gatewayName string, externalHostname string) ([]*v1.HTTPRoute, error) {
	...
	return v1.BuildHTTPRoutes(config, rule, pseudoService, pseudoServicePort, nil, node.Domain, buildCluster), nil
}
