...
func BuildSidecarOutboundVirtualHosts(node *model.Proxy, push *model.PushContext,
	...
	for _, svc := range services {
		if listenerPort == 0 {
			...
			arrHostName = append(arrHostName, svc.Hostname)
	...
	if listenerPort != 80 {
		...
		hostNameByNamespace := make(map[string][]host.Name)
		// No namespace of virtualservices should be respected for OutboundVirtualHosts generation
		hostNameByNamespace["*"] = arrHostName
		virtualServices = model.SelectVirtualServices(virtualServices, hostNameByNamespace)
	}
