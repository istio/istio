...
import (
	...
	"istio.io/istio/pkg/test/framework/components/endpointpicker"
...
func TestInferencePoolMultipleTargetPorts(t *testing.T) {
	framework.
		...
		Run(func(ctx framework.TestContext) {
			...
			// Deploy the endpoint picker (EPP) service
			// This is an external processor that selects endpoints based on request headers
			epp := endpointpicker.NewOrFail(ctx, ctx, endpointpicker.Config{
				Namespace:   ns,
				ServiceName: "mock-epp",
				Port:        9002,
			})
...
spec:
  ...
  endpointPickerRef:
    ...
    port:
      number: %d
`, ns.Name(), epp.ServiceName(), epp.Port())
...
`, ns.Name(), ns.Name())
			...
			retry.UntilSuccessOrFail(ctx, func() error {
				...
				if svc.Labels["istio.io/inferencepool-extension-service"] != epp.ServiceName() {
					return fmt.Errorf("missing or incorrect EPP service label, expected %s, got %s",
						epp.ServiceName(), svc.Labels["istio.io/inferencepool-extension-service"])
				}

				expectedPort := fmt.Sprintf("%d", epp.Port())
