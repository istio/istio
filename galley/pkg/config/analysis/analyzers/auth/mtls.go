...
func (s *MTLSAnalyzer) Metadata() analysis.Metadata {
	return analysis.Metadata{
		...
		Inputs: collection.Names{
			...
			metadata.IstioAuthenticationV1Alpha1Meshpolicies.Name,
			metadata.IstioAuthenticationV1Alpha1Policies.Name,
			metadata.IstioMeshV1Alpha1MeshConfig.Name,
			metadata.IstioNetworkingV1Alpha3Destinationrules.Name,
		},
	}
}
...
func (s *MTLSAnalyzer) Analyze(c analysis.Context) {
	...
	c.ForEach(metadata.K8SCoreV1Services.Name, func(r *resource.Instance) bool {
		...
		c.ForEach(metadata.K8SCoreV1Pods.Name, func(pr *resource.Instance) bool {
	...
	meshPolicyResource := c.Find(metadata.IstioAuthenticationV1Alpha1Meshpolicies.Name, resource.NewFullName("", "default"))
	if meshPolicyResource != nil {
		err := pc.AddMeshPolicy(meshPolicyResource, meshPolicyResource.Message.(*v1alpha1.Policy))
		if err != nil {
			c.Report(metadata.IstioAuthenticationV1Alpha1Meshpolicies.Name, msg.NewInternalError(meshPolicyResource, err.Error()))
			return
		}
	}

	c.ForEach(metadata.IstioAuthenticationV1Alpha1Policies.Name, func(r *resource.Instance) bool {
		...
		if err != nil {
			...
			if missingPortNameErr, ok := err.(mtls.NamedPortInPolicyNotFoundError); ok {
				c.Report(metadata.IstioAuthenticationV1Alpha1Meshpolicies.Name,
					msg.NewPolicySpecifiesPortNameThatDoesntExist(r, missingPortNameErr.PortName, missingPortNameErr.FQDN))
				return true
			}
			c.Report(metadata.IstioAuthenticationV1Alpha1Meshpolicies.Name, msg.NewInternalError(r, err.Error()))
			return false
		}
		return true
	})
	...
	c.ForEach(metadata.IstioNetworkingV1Alpha3Destinationrules.Name, func(r *resource.Instance) bool {
		ns := r.Metadata.FullName.Namespace
		namespaces[ns] = struct{}{}
	...
	for _, ts := range targetServices {
		...
		if _, ok := fqdnsWithoutSidecars[ts.FQDN()]; ok {
			...
			if err != nil {
				c.Report(metadata.IstioAuthenticationV1Alpha1Policies.Name, msg.NewInternalError(nil, err.Error()))
				return
			}
		}
		...
		for ns := range namespaces {
			...
			if (tsPolicy.MTLSMode == mtls.ModeStrict && !mtlsUsed) ||
				...
				if _, ok := fqdnsWithoutSidecars[ts.FQDN()]; ok {
					c.Report(metadata.IstioNetworkingV1Alpha3Destinationrules.Name,
