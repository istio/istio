...
`, "")
  ...
  istioctl waypoint list --namespace default`,
	...
	waypointApplyCmd.Flags().StringSliceVar(&namespaceSelector, "cross-namespace-selector", nil, "Set label selector for routable namespace")
	waypointGenerateCmd.Flags().StringVarP(&revision, "revision", "r", "", "The revision to label the waypoint with")
	waypointGenerateCmd.Flags().StringSliceVar(&namespaceSelector, "cross-namespace-selector", nil, "Set label selector for routable namespace")
