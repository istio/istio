	...
	go func() {
		...
		if m.distributeCACert && (shouldLead || configCluster) {
			if features.EnableClusterTrustBundles {
				...
				if namespaceControllerElectionID(m.revision) == leaderelection.NamespaceController {
					close(activateNamespaceControllerCh)
				} else {
...
func (m *Multicluster) checkAndRunNonRevisionNamespaceControllerElectionIfRequired(
	...
	m.s.RunComponentAsyncAndWait("namespace controller legacy lock", func(componentStop <-chan struct{}) error {
		...
		leaderelection.
			NewLeaderElectionMulticluster(systemNamespace, m.serverID, leaderelection.NamespaceController, m.revision, remote, client).
			AddRunFunction(func(leaderStop <-chan struct{}) {
