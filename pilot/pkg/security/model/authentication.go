	...
	if len(tlsOpts.CredentialNames) > 0 {
		...
		for _, name := range tlsOpts.CredentialNames {
			if strings.HasSuffix(name, SdsCaSuffix) {
				...
				tlsContext.TlsCertificateSdsSecretConfigs = append(tlsContext.TlsCertificateSdsSecretConfigs, ConstructSdsSecretConfigForCredential(name, credentialSocketExist))
			}
		}
