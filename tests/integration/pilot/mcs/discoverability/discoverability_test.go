				...
				t.NewSubTest(ht.String()).Run(func(t framework.TestContext) {
					runForAllClusterCombinations(t, func(t framework.TestContext, from echo.Instance, to echo.Target) {
						...
						if ht == hostTypeClusterLocal {
							...
							checker = checkClustersReached(t.AllClusters(), expectedClusters, t)
						} else {
			...
			for _, ht := range hostTypes {
				t.NewSubTest(ht.String()).Run(func(t framework.TestContext) {
					runForAllClusterCombinations(t, func(t framework.TestContext, from echo.Instance, to echo.Target) {
						...
						callAndValidate(t, ht, from, to, checkClustersReached(t.AllClusters(), expectedClusters, t))
					})
				})
			}
		})
...
func checkClustersReached(allClusters cluster.Clusters, clusters cluster.Clusters, t framework.TestContext) echo.Checker {
	return check.And(
		check.OK(),
		check.ReachedClusters(allClusters, clusters, t))
}
