...
func (sf *SecretFetcher) configMapAdded(obj interface{}) {
	...
	if !strings.HasPrefix(resourceName, spiffeTrustBundlePrefix) {
		secretFetcherLog.Warnf("ConfigMap %s is not a trust bundle, skip adding it", resourceName)
		return
	}
	...
	secretFetcherLog.Warnf("secret %s is added as a root cert", caNewSecret.ResourceName)
	if sf.AddCache != nil {
