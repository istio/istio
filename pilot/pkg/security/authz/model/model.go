...
func (m Model) Generate(forTCP, forAllow bool) (*rbacpb.Policy, error) {
	var permissions []*rbacpb.Permission
	for _, rl := range m.permissions {
		permission, err := generatePermission(rl, forTCP, forAllow)
		if err != nil {
			return nil, err
		}
	...
	for _, rl := range m.principals {
		principal, err := generatePrincipal(rl, forTCP, forAllow)
		if err != nil {
			return nil, err
		}
...
func generatePermission(rl ruleList, forTCP, forAllow bool) (*rbacpb.Permission, error) {
	var and []*rbacpb.Permission
	for _, r := range rl.rules {
		ret, err := r.permission(forTCP, forAllow)
		if err != nil {
			return nil, err
		}
		and = append(and, ret...)
...
func generatePrincipal(rl ruleList, forTCP, forAllow bool) (*rbacpb.Principal, error) {
	var and []*rbacpb.Principal
	for _, r := range rl.rules {
		ret, err := r.principal(forTCP, forAllow)
		if err != nil {
			return nil, err
		}
		and = append(and, ret...)
...
func (r rule) permission(forTCP, forAllow bool) ([]*rbacpb.Permission, error) {
	var permissions []*rbacpb.Permission
	var or []*rbacpb.Permission
	for _, value := range r.values {
		p, err := r.g.permission(r.key, value, forTCP)
		if err := r.checkError(forAllow, err); err != nil {
			return nil, err
		}
		if p != nil {
			or = append(or, p)
...
func (r rule) principal(forTCP, forAllow bool) ([]*rbacpb.Principal, error) {
	var principals []*rbacpb.Principal
...
func (r rule) checkError(forAllow bool, err error) error {
	if forAllow {
