...
SECURITY_GO_BINS:=${ISTIO_OUT}/node_agent {ISTIO_OUT}/node_agent_k8s ${ISTIO_OUT}/istio_ca ${ISTIO_OUT}/flexvolume
$(SECURITY_GO_BINS):
	bin/gobuild.sh $@ istio.io/istio/pkg/version ./security/cmd/$(@F)

.PHONY: build
# Build will rebuild the go binaries.
build: depend $(PILOT_GO_BINS_SHORT) mixc mixs node_agent node_agent_k8s istio_ca flexvolume istioctl gals
...
.PHONY: node_agent node_agent_k8s istio_ca flexvolume
node_agent node_agent_k8s istio_ca flexvolume:
