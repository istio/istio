...
func adjustInitContainerUser(finalPod *corev1.Pod, originalPod *corev1.Pod) {
	...
	initContainer := FindContainer(InitContainerName, finalPod.Spec.InitContainers)
	if initContainer == nil {
		initContainer = FindContainer(ValidationContainerName, finalPod.Spec.InitContainers)
	}
	if initContainer == nil {
		// should not happen
		return
	...
	// Make sure the init container runs with the same uid/gid as the proxy
