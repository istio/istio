...
// RunTproxyTests verifies that when TPROXY interception mode is used, the server
// sees the client's original source IP rather than the sidecar proxy IP.
func RunTproxyTests(t framework.TestContext, apps deployment.SingleNamespaceView) {
	t.Helper()
