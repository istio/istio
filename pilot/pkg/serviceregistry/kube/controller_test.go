...
func TestController_getPodAZ(t *testing.T) {
	t.Parallel()
	pod1 := generatePod("128.0.1.1", "pod1", "nsA", "", "node1", map[string]string{"app.kubernetes.io/name": "prod-app"}, map[string]string{})
	pod2 := generatePod("128.0.1.2", "pod2", "nsB", "", "node2", map[string]string{"app.kubernetes.io/name": "prod-app"}, map[string]string{})
	testCases := []struct {
		name   string
		pods   []*v1.Pod
		nodes  []*v1.Node
...
func TestGetProxyServiceInstances(t *testing.T) {
	controller, fx := newFakeController(t)
	defer controller.Stop()
	p := generatePod("128.0.0.1", "pod1", "nsa", "foo", "node1", map[string]string{"app.kubernetes.io/name": "test-app"}, map[string]string{})
	...
	createService(controller, "svc1", "nsa",
		...
		[]int32{8080}, map[string]string{"app.kubernetes.io/name": "prod-app"}, t)
	ev := fx.Wait("service")
	if ev == nil {
...
func TestController_GetIstioServiceAccounts(t *testing.T) {
	...
	pods := []*v1.Pod{
		generatePod("128.0.0.1", "pod1", "nsA", sa1, "node1", map[string]string{"app.kubernetes.io/name": "test-app"}, map[string]string{}),
		generatePod("128.0.0.2", "pod2", "nsA", sa2, "node2", map[string]string{"app.kubernetes.io/name": "prod-app"}, map[string]string{}),
		generatePod("128.0.0.3", "pod3", "nsB", sa3, "node1", map[string]string{"app.kubernetes.io/name": "prod-app"}, map[string]string{}),
	}
	...
	createService(controller, "svc1", "nsA",
		map[string]string{
			...
			CanonicalServiceAccountsAnnotation: canonicalSaOnVM},
		[]int32{8080}, map[string]string{"app.kubernetes.io/name": "prod-app"}, t)
	createService(controller, "svc2", "nsA", nil, []int32{8080}, map[string]string{"app.kubernetes.io/name": "staging-app"}, t)
	<-fx.Events
...
func TestWorkloadHealthCheckInfoPrometheusScrape(t *testing.T) {
	...
	pods := []*v1.Pod{
		generatePod("128.0.1.6", "pod1", "nsA", "", "node1", map[string]string{"app.kubernetes.io/name": "test-app"},
...
func TestWorkloadHealthCheckInfoPrometheusPath(t *testing.T) {
	...
	pods := []*v1.Pod{
		generatePod("128.0.1.7", "pod1", "nsA", "", "node1", map[string]string{"app.kubernetes.io/name": "test-app"},
...
func TestWorkloadHealthCheckInfoPrometheusPort(t *testing.T) {
	...
	pods := []*v1.Pod{
		generatePod("128.0.1.8", "pod1", "nsA", "", "node1", map[string]string{"app.kubernetes.io/name": "test-app"},
